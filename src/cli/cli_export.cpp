...
// Provides helper data that exists during execution of
// cli_asyncExportDocuments() function
struct Helper : public QObject
{
    // Task manager object to be used
    TaskManager taskMgr;
    // Counter decremented for each finished export task, when 0 is reached then
    // quit
    ...
    // Mapping between a task id and corresponding width of the progress line in
    // console
...
void printTaskProgress(Helper *helper, TaskId taskId)
{
    ...
    const int lineWidthOld =
        itLineFound != helper->mapTaskLineWidth.cend() ? itLineFound->second : printWidth - 1;
...
bool importInDocument(DocumentPtr doc, const CliExportArgs &args, Helper *helper,
                      TaskProgress *progress)
{
    auto appModule = AppModule::get();

    // If export operation targets some mesh format then force meshing of imported
    // BRep shapes
    ...
    const bool okImport =
        appModule->ioSystem()
            ...
            .execute();
    helper->taskMgr.setTitle(progress->taskId(), okImport ? CliExport::textIdTr("Imported") :
...
void exportDocument(const DocumentPtr &doc, const FilePath &filepath, Helper *helper,
                    TaskProgress *progress)
{
    auto appModule = AppModule::get();
    MessageCollecter errorCollect;
    ...
    const bool okExport = appModule->ioSystem()
                              ...
                              .withTaskProgress(progress)
                              .execute();
    const std::string strFilename = filepath.filename().string();
    const std::string msg =
        okExport ? fmt::format(fmt::runtime(CliExport::textIdTr("Exported {}")), strFilename) :
                   errorCollect.asString(" ");
...
void cli_asyncExportDocuments(const ApplicationPtr &app, const CliExportArgs &args,
                              std::function<void(int)> fnContinuation)
{
    ...
    taskMgr->signalStarted.connectSlot(
        [=](TaskId taskId)
        {
    ...
    bool okImport = true;
    const TaskId importTaskId = taskMgr->newTask(
        [&](TaskProgress *progress) { okImport = importInDocument(doc, args, helper, progress); });
    ...
    for (const FilePath &filepath : args.filesToExport)
    {
        const TaskId taskId = taskMgr->newTask(
            [=](TaskProgress *progress) { exportDocument(doc, filepath, helper, progress); });
        const std::string strFilename = filepath.filename().string();
        helper->mapTaskStatus.insert({taskId, std::make_unique<TaskStatus>()});
        taskMgr->setTitle(
            taskId, fmt::format(fmt::runtime(CliExport::textIdTr("Exporting {}...")), strFilename));
    }
