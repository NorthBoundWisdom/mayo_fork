/*
  ...
  SPDX-FileCopyrightText: 2021-2022 Klarälvdalens Datakonsult AB, a KDAB Group
  company <info@kdab.com> Author: Sean Harmer <sean.harmer@kdab.com>
...
{
//
// This class defines a virtual interface, that the Signal this ConnectionHandle
// refers to must implement. It allows ConnectionHandle to refer to this
// non-template class, which then dispatches to the template implementation
// using virtual function calls. It allows ConnectionHandle to be a non-template
// class.
 ...
 * manage the connection by disconnecting, (un)blocking it and checking its
 *state.
 *
...
public:
     ...
     * In this case the ConnectionHandle will not reference any active connection
     *(i.e. isActive() will return false), and not belong to any Signal.
     **/
     ...
     * Disconnect the slot.
     *
     * When this function is called, the function that was passed to
     *Signal::connect to create this ConnectionHandle will no longer be called
     *when the Signal is emitted.
     *
     * If the ConnectionHandle is not active or the connection has already been
     *disconnected, nothing happens.
     *
     * After this call, the ConnectionHandle will be inactive (i.e. isActive()
     *returns false) and will no longer belong to any Signal (i.e. belongsTo
     *returns false).
     **/
    void disconnect()
    {
     ...
     *
     * To temporarily block a connection, consider using an instance of
     *ConnectionBlocker, which offers a RAII-style implementation that makes sure
     *the connection is always returned to its original state.
     *
     * @param blocked The new blocked state of the connection.
     * @return whether the connection was previously blocked.
     * @throw std::out_of_range Throws if the connection is not active (i.e.
     ...
     *
     * @return true if this ConnectionHandle refers to a connection within the
     *given Signal
     **/
    template <typename... Args>
 ...
 *
 * KDBindings::Signal recreates the <a
 * href="https://doc.qt.io/qt-5/signalsandslots.html">Qt's Signals & Slots
 * mechanism</a> in pure C++17. A Signal can be used to notify any number of
 * slots that a certain event has occurred.
 *
 * The slot can be almost any callable object, including member functions and
 * lambdas.
 *
 * This connection happens in a type-safe manner, as a slot can only be
 * connected to a Signal when the arguments of the slot match the values the
 * Signal emits.
 *
...
class Signal
{
    ...
    // The Signal::Impl class exists, so Signals can be implemented in a
    // PIMPL-like way. This allows us to easily move Signals without losing their
    // ConnectionHandles, as well as making an unconnected Signal only
    // sizeof(shared_ptr).
    ...
    public:
        ...
        bool blockConnection(const Private::GenerationalIndex &id, bool blocked) override
        {
            ...
            else
            {
                throw std::out_of_range(
                    ...
                    "the connection was deleted before!");
            }
        }
        ...
        void emit(Args... p) const
        {
            ...
            // but adding new connections to a signal inside a slot will still be
            // undefined behaviour
...
public:
     ...
     * will no longer be active (i.e. ConnectionHandle::isActive will return
     * false).
     */
    ~Signal()
    {
    ...
    /**
     * A template overload of Signal::connect that makes it easier to connect
     *arbitrary functions to this Signal. It connects a function to this Signal,
     *binds any provided arguments to that function and discards any values
     *emitted by this Signal that aren't needed by the resulting function.
     *
     ...
     * void (std::vector<int>::*push_back)(const int &) =
     *&std::vector<int>::push_back; signal.connect(push_back, &numbers);
     *
     ...
     *
     * @return An instance of a Signal::ConnectionHandle that refers to this
     *connection. Warning: When connecting a member function you must use the
     *returned ConnectionHandle to disconnect when the object containing the slot
     *goes out of scope!
     **/
    // The enable_if_t makes sure that this connect function specialization is
    // only available if we provide a function that cannot be otherwise converted
    // to a std::function<void(Args...)>, as it otherwise tries to take precedence
     ...
     * longer be active afterwards. (i.e. ConnectionHandle::isActive will return
     * false).
     */
    void disconnectAll()
    {
        if (m_impl)
        {
            ...
            // Once all connections are disconnected, we can release ownership of the
            // Impl. This does not destroy the Signal itself, just the Impl object. If
            // another slot is connected, another Impl object will be constructed.
            m_impl.reset();
        }
    ...
    void emit(Args... p) const
    {
        ...
        // if m_impl is nullptr, we don't have any slots connected, don't bother
        // emitting
    }
...
private:
    ...
    // shared_ptr is used here instead of unique_ptr, so ConnectionHandle
    // instances can use a weak_ptr to check if the Signal::Impl they reference is
    // still alive.
    //
    // This makes Signals easily copyable in theory, but the semantics of this are
    // unclear. Copying could either simply copy the shared_ptr, which means the
    // copy would share the connections of the original, which is possibly
    // unintuitive, or the Impl would have to be copied as well. This would
    // however leave connections without handles to disconnect them. So copying is
    // forbidden for now.
    //
...
/**
 * @brief A ScopedConnection is a RAII-style way to make sure a Connection is
 * disconnected.
 *
 * When the ScopedConnections scope ends, the connection this ScopedConnection
 * guards will be disconnected.
 *
 * Example:
...
public:
     ...
     * Therefore it does nothing when it is destructed, unless a ConnectionHandle
     * is assigned to it.
     */
    ...
    /**
     * When a ConnectionHandle is destructed it disconnects the connection it
     * guards.
     */
    ~ScopedConnection()
    {
...
/**
 * @brief A ConnectionBlocker is a convenient RAII-style mechanism for
 * temporarily blocking a connection.
 *
 * When a ConnectionBlocker is constructed, it will block the connection.
 *
 * When it is destructed, it will return the connection to the blocked state it
 * was in before the ConnectionBlocker was constructed.
 *
 * Example:
 * - @ref 08-managing-connections/main.cpp
 */
...
public:
    /**
     * Constructs a new ConnectionBlocker and blocks the connection this
     * ConnectionHandle refers to.
     *
     * @throw std::out_of_range If the connection is not active (i.e.
     * ConnectionHandle::isActive() returns false).
     */
    ...
    /**
     * Destructs the ConnectionBlocker and returns the connection into the blocked
     * state it was in before the ConnectionBlocker was constructed.
     */
    ~ConnectionBlocker()
    {
 ...
 *
 * A simple example of how to create a KDBindings::Signal and connect a lambda
 * to it.
 *
 ...
 *
 * An example of how to connect a member function with arguments to a
 * KDBindings::Signal.
 *
 * The output of this example is:
 * ```
 ...
 *
 * An example of how to use the KDBindings::Signal::connect() overloaded
 * function for advanced slot connections.
 *
 * The output of this example is:
 * ```
 * Hello World!
