...
fastfloat_really_inline parsed_number_string parse_number_string(const char *p, const char *pend,
                                                                 chars_format fmt) noexcept
{
    ...
    if ((p != pend) && (*p == '.'))
    {
        ...
        {
            i = i * 100000000 +
                parse_eight_digits_unrolled(p); // in rare cases, this will overflow, but that's ok
            p += 8;
            if ((p + 8 <= pend) && is_made_of_eight_digits_fast(p))
            {
                i = i * 100000000 + parse_eight_digits_unrolled(
...
// This function could be optimized. In particular, we could stop after 19
// digits and try to bail out. Furthermore, we should be able to recover the
// computed exponent from the pass in parse_number_string.
fastfloat_really_inline decimal parse_decimal(const char *p, const char *pend) noexcept
{
    ...
    // In very rare cases, we may have fewer than 19 digits, we want to be able to
    // reliably assume that all digits up to max_digit_without_overflow have been
    // initialized.
