...
class AppModule : public IO::ParametersProvider, public PropertyValueConversion, public Messenger
{
...
public:
    ...
    void addLibraryInfo(std::string_view libName, std::string_view version,
                        std::string_view versionDetails = "");
    Span<const LibraryInfo> libraryInfoArray() const;

    // Logging
    void clearMessageLog();
    Span<const Message> messageLog() const
    {
        return m_messageLog;
    }
    ...
    QSize recentFileThumbnailSize() const
    {
        return {190, 150};
    }
