...
public:
        ...
        : m_label(treeNode.label())
    {
        ...
        // Material
        {
            const TDF_Label labelPart =
                XCaf::isShapeReference(label) ? XCaf::shapeReferred(label) : label;
        ...
        if (XCaf::isShapeReference(label))
        {
            ...
            m_propertyReferredName.setValue(
                to_stdString(CafUtils::labelAttrStdName(m_labelReferred)));
...
{
    return GraphicsShapeObjectDriver::shapeSupportStatus(treeNode.label()) ==
...
public:
    Properties(const DocumentTreeNode &treeNode)
    {
        ...
        IMeshAccess_visitMeshes(treeNode,
                                [&](const IMeshAccess &access) { mesh = access.triangulation(); });
        ...
        m_propertyVolume.setQuantity(MeshUtils::triangulationVolume(mesh) *
                                     Quantity_CubicMillimeter);
...
{
    return GraphicsMeshObjectDriver::meshSupportStatus(treeNode.label()) ==
           GraphicsObjectDriver::Support::Complete;
}
...
public:
    Properties(const DocumentTreeNode &treeNode)
    {
        ...
        const bool hasAttrData =
            !attrPointCloudData.IsNull() && !attrPointCloudData->points().IsNull();
        m_propertyPointCount.setValue(hasAttrData ? attrPointCloudData->points()->VertexNumber() :
                                                    0);
        m_propertyHasColors.setValue(hasAttrData ? attrPointCloudData->points()->HasVertexColors() :
                                                   false);
        if (hasAttrData)
        {
            Bnd_Box bndBox;
...
{
    return GraphicsPointCloudObjectDriver::pointCloudSupportStatus(treeNode.label()) ==
           GraphicsObjectDriver::Support::Complete;
}
