...
private:
        ...
        : PropertyGroup(AppModule::get()->settings())
    {
        ...
        settings->addSetting(&this->instanceNameFormat,
                             appModule->properties()->groupId_application);
        settings->addResetFunction(appModule->properties()->groupId_application,
                                   [=] { this->instanceNameFormat.setValue(NameFormat::Product); });
        this->instanceNameFormat.mutableEnumeration().changeTrContext(
            WidgetModelTreeBuilder_Xde::textIdContext());
    }
};

bool WidgetModelTreeBuilder_Xde::supportsDocumentTreeNode(const DocumentTreeNode &node) const
{
    return GraphicsShapeObjectDriver::shapeSupportStatus(node.label()) ==
           GraphicsObjectDriver::Support::Complete;
}

void WidgetModelTreeBuilder_Xde::refreshTextTreeItem(const DocumentTreeNode &node,
                                                     QTreeWidgetItem *treeItem)
{
    ...
    {
        XCAFDoc_ShapeTool::GetUsers(XCaf::shapeReferred(labelNode), seqLabelRefresh,
...
QTreeWidgetItem *WidgetModelTreeBuilder_Xde::guiCreateXdeTreeNode(QTreeWidgetItem *guiParentNode,
                                                                  const DocumentTreeNode &node)
{
...
QTreeWidgetItem *WidgetModelTreeBuilder_Xde::buildXdeTree(QTreeWidgetItem *treeItem,
                                                          const DocumentTreeNode &node)
{
...
QString WidgetModelTreeBuilder_Xde::referenceItemText(const TDF_Label &instanceLabel,
                                                      const TDF_Label &productLabel) const
{
    ...
    const QByteArray strTemplate =
        Module::toInstanceNameTemplate(Module::get()->instanceNameFormat);
    QString itemText = QString::fromUtf8(strTemplate);
    itemText.replace("%instance", instanceName).replace("%product", productName);
    return itemText;
}

QTreeWidgetItem *WidgetModelTreeBuilder_Xde::findTreeItem(QTreeWidgetItem *parentTreeItem,
