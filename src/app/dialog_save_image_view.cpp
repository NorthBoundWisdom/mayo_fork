...
{
    const QImage img(occImg.Data(), int(occImg.Width()), int(occImg.Height()),
                     int(occImg.SizeRowBytes()), QImage::Format_RGBA8888);
    return img;
}
...
void DialogSaveImageView::saveFile()
{
    ...
    const QString fileName =
        QFileDialog::getSaveFileName(this, tr("Select image file"), QString(),
                                     listFormat.join(QLatin1String(";;")), &selectedFormat);
    if (!fileName.isEmpty())
    {
        auto itFound = mapFilterFormat.find(selectedFormat);
        const char *format =
            itFound != mapFilterFormat.cend() ? itFound.value().constData() : nullptr;
...
{
    img->SetTopDown(true);
    const bool ok = m_view->ToPixMap(*img, m_ui->edit_Width->value(), m_ui->edit_Height->value(),
                                     Graphic3d_BT_RGBA, m_ui->checkBox_KeepRatio->isChecked());
    return ok;
}
