 ...
 * to be drawn. If you have a custom delegate (eg. a subclass of
 * QStyledItemDelegate) then just call at some point ItemViewButtons::paint()
 * inside the delegate's paint() method :
 * \code
 ...
 * \endcode
 *
 * If you do not want to modify your delegate class then
 * createProxyItemDelegate() might be the right option : this will create a new
 * delegate around yours with the paint() method correctly called.
 *
 * If the item-view does not use any delegate then just call
 * installDefaultItemDelegate()
 *
