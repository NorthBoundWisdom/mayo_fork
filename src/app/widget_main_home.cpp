...
void WidgetMainHome::initialize(const CommandContainer *cmdContainer)
{
    ...
    QObject::connect(m_ui->widget_HomeFiles, &WidgetHomeFiles::newDocumentRequested,
                     cmdContainer->findCommand(CommandNewDocument::Name), &Command::execute);
    QObject::connect(m_ui->widget_HomeFiles, &WidgetHomeFiles::openDocumentsRequested,
                     cmdContainer->findCommand(CommandOpenDocuments::Name), &Command::execute);
    QObject::connect(m_ui->widget_HomeFiles, &WidgetHomeFiles::recentFileOpenRequested, this,
                     [=](const QFileInfo &fp)
                     { FileCommandTools::openDocument(appContext, filepathFrom(fp)); });
}
