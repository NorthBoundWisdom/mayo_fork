...
public:
    ...
    int pixelMetric(QStyle::PixelMetric metric, const QStyleOption *option,
    ...
    , m_controller(new WidgetOccViewController(m_qtOccView))
{
    ...
    m_btnEditClipping =
        this->createViewBtn(widgetBtnsContents, Theme::Icon::ClipPlane, tr("Edit clip planes"));
    m_btnEditClipping->setCheckable(true);

    m_btnExplode =
        this->createViewBtn(widgetBtnsContents, Theme::Icon::Multiple, tr("Explode assemblies"));
    m_btnExplode->setCheckable(true);

    m_btnMeasure =
        this->createViewBtn(widgetBtnsContents, Theme::Icon::Measure, tr("Measure shapes"));
    ...
    gfxScene->signalRedrawRequested.connectSlot(
    ...
    QObject::connect(m_btnFitAll, &ButtonFlat::clicked, this,
                     [=]
                     {
                         m_guiDoc->runViewCameraAnimation(
                             [=](OccHandle<V3d_View> view)
                             {
                                 auto bndBoxFlags = GuiDocument::OnlySelectedGraphics |
                                                    GuiDocument::OnlyVisibleGraphics;
                                 GraphicsUtils::V3dView_fitAll(
                                     view, this->guiDocument()->graphicsBoundingBox(bndBoxFlags));
                             });
                     });
    QObject::connect(m_btnGrid, &ButtonFlat::checked, this, &WidgetGuiDocument::toggleWidgetGrid);
    QObject::connect(m_btnEditClipping, &ButtonFlat::checked, this,
                     &WidgetGuiDocument::toggleWidgetClipPlanes);
    QObject::connect(m_btnExplode, &ButtonFlat::checked, this,
                     &WidgetGuiDocument::toggleWidgetExplode);
    QObject::connect(m_btnMeasure, &ButtonFlat::checked, this,
                     &WidgetGuiDocument::toggleWidgetMeasure);
    m_controller->signalDynamicActionStarted.connectSlot([=]
                                                         { m_guiDoc->stopViewCameraAnimation(); });
    m_controller->signalViewScaled.connectSlot([=] { m_guiDoc->stopViewCameraAnimation(); });
    m_controller->signalMouseButtonClicked.connectSlot(
        [=](Aspect_VKeyMouse btn)
        {
            if (btn == Aspect_VKeyMouse_LeftButton &&
    ...
    m_controller->signalMultiSelectionToggled.connectSlot(
        [=](bool on)
        {
            auto mode =
    ...
    m_guiDoc->viewCameraAnimation()->setBackend(
        std::make_unique<QtAnimationBackend>(QEasingCurve::OutExpo));
    m_guiDoc->viewCameraAnimation()->setRenderFunction(
        [=](const OccHandle<V3d_View> &view)
        {
            if (view == m_qtOccView->v3dView())
                m_qtOccView->redraw();
...
void WidgetGuiDocument::toggleWidgetGrid(bool on)
{
    if (!m_widgetGrid && on)
    {
        ...
        QObject::connect(
            ...
            [=] { adjustWidgetSize(m_widgetGrid); }, Qt::QueuedConnection);
    }
...
void WidgetGuiDocument::toggleWidgetMeasure(bool on)
{
    if (!m_widgetMeasure && on)
    {
        ...
        QObject::connect(
            m_widgetMeasure, &WidgetMeasure::sizeAdjustmentRequested, container,
            [=] { adjustWidgetSize(m_widgetMeasure); }, Qt::QueuedConnection);
    }
...
void WidgetGuiDocument::layoutWidgetPanel(QWidget *panel)
{
    auto fnPanelPos = [=](QWidget *panel) -> QPoint
    {
        ...
        {
        case Aspect_TOTP_LEFT_UPPER: return QPoint(ctrlRect.left(), ctrlRect.bottom() + margin);
        ...
        case Aspect_TOTP_RIGHT_LOWER:
            return QPoint(this->width() - panel->width(),
                          ctrlRect.top() - panel->height() - margin);
        default: return QPoint(margin, ctrlRect.bottom() + margin);
        } // endswitch
    };
...
ButtonFlat *WidgetGuiDocument::createViewBtn(QWidget *parent, Theme::Icon icon,
                                             const QString &tooltip) const
{
    const QColor bkgndColor = m_qtOccView->supportsWidgetOpacity() ?
                                  Qt::transparent :
...
void WidgetGuiDocument::recreateMenuViewProjections(QWidget *container)
{
    ...
    if (m_guiDoc->viewTrihedronMode() == GuiDocument::ViewTrihedronMode::AisViewCube)
    {
        ...
        const QString strTemplateTooltip = tr("<b>Left-click</b>: popup menu of pre-defined views\n"
        ...
        menuBtnView->setStyleSheet(
            QString("QMenu { background:%1; border: 0px }").arg(strPanelBkgndColor));
        menuBtnView->setWindowFlags(menuBtnView->windowFlags() | Qt::FramelessWindowHint |
        ...
        QObject::connect(
            btnViewMenu, &ButtonFlat::clicked, this,
            [=]
            {
...
void WidgetGuiDocument::layoutViewControls()
{
    ...
    auto fnGetViewControlsPos = [=]() -> QPoint
    {
        if (m_guiDoc->viewTrihedronMode() == GuiDocument::ViewTrihedronMode::AisViewCube)
        {
            ...
            const int viewCubeBndSize =
                m_guiDoc->aisViewCubeBoundingSize() / m_guiDoc->devicePixelRatio();
            const int ctrlHeight = btnSize;
            const int ctrlXOffset = margin;
            switch (m_guiDoc->viewTrihedronCorner())
            {
            case Aspect_TOTP_LEFT_UPPER: return {ctrlXOffset, viewCubeBndSize + margin};
            case Aspect_TOTP_RIGHT_UPPER:
