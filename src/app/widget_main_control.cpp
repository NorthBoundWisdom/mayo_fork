    ...
    , m_docFilesWatcher(new DocumentFilesWatcher(guiApp->application(), this))
{
    ...
    // IMPORTANT:
    // GuiDocumentListModel object must be created *BEFORE* signal/slot connection
    // between GuiApplication::signalGuiDocumentAdded and
    // WidgetMainControl::onGuiDocumentAdded() onGuiDocumentAdded() is changing
    // the currentIndex of combo_GuiDocuments but the GuiModel must have been
    // updated before
    ...
    QObject::connect(m_ui->combo_GuiDocuments, qOverload<int>(&QComboBox::currentIndexChanged),
                     this, &WidgetMainControl::onCurrentDocumentIndexChanged);
    QObject::connect(m_ui->widget_FileSystem, &WidgetFileSystem::locationActivated, this,
                     &WidgetMainControl::onWidgetFileSystemLocationActivated);
    // ...
    QObject::connect(m_ui->combo_LeftContents, qOverload<int>(&QComboBox::currentIndexChanged),
                     this, &WidgetMainControl::onLeftContentsPageChanged);
    QObject::connect(m_ui->listView_OpenedDocuments, &QListView::clicked, this,
                     [=](const QModelIndex &index) { this->setCurrentDocumentIndex(index.row()); });
    QObject::connect(m_ui->splitter_Main, &QSplitter::splitterMoved, this,
                     &WidgetMainControl::onSplitterMainMoved);
    ...
    guiApp->signalGuiDocumentErased.connectSlot(&WidgetMainControl::onGuiDocumentErased, this);

    // Document files monitoring
    auto appModule = AppModule::get();
    const auto &propActionOnDocumentFileChange =
        appModule->properties()->actionOnDocumentFileChange;
    m_docFilesWatcher->enable(propActionOnDocumentFileChange != ActionOnDocumentFileChange::None);
    appModule->settings()->signalChanged.connectSlot(
        [&](const Property *property)
        {
            if (property == &propActionOnDocumentFileChange)
            {
                m_docFilesWatcher->enable(propActionOnDocumentFileChange !=
                                          ActionOnDocumentFileChange::None);
...
void WidgetMainControl::initialize(const CommandContainer *cmdContainer)
{
    ...
    m_ui->btn_CloseLeftSideBar->setDefaultAction(
        fnFindAction(CommandLeftSidebarWidgetToggle::Name));
    ...
    QObject::connect(m_listViewBtns, &ItemViewButtons::buttonClicked, this,
                     [=](int btnId, QModelIndex index)
                     {
                         if (btnId == 1)
                         {
...
void WidgetMainControl::setWidgetLeftSideBarWidthFactor(double factor)
{
    ...
    m_ui->splitter_Main->setSizes(
        {int(leftFactor * mainWidth), int((1. - leftFactor) * mainWidth)});
...
QMenu *WidgetMainControl::createMenuModelTreeSettings()
{
    ...
    auto appModule = AppModule::get();
    QAction *action =
        menu->addAction(to_QString(appModule->properties()->linkWithDocumentSelector.name().tr()));
    ...
    if (spanAppItem.size() == 1)
    {
        ...
        else if (appItem.isDocumentTreeNode())
        {
            ...
            if (dataProps)
            {
                ...
                dataProps->signalPropertyChanged.connectSlot(
            ...
            if (commonGfxDriver)
            {
                ...
                if (gfxProps)
                {
                    ...
                    gfxProps->signalPropertyChanged.connectSlot(
...
{
    return m_ui->widget_LeftHeader->findChild<QWidget *>("LeftHeaderPlaceHolder",
...
void WidgetMainControl::reloadDocumentAfterChange(const DocumentPtr &doc)
{
    ...
    const auto &propActionOnDocumentFileChange =
        AppModule::get()->properties()->actionOnDocumentFileChange;
    ...
    if (propActionOnDocumentFileChange == ActionOnDocumentFileChange::ReloadIfUserConfirm)
    {
        ...
        auto msgBox =
            new QMessageBox(QMessageBox::Question, tr("Question"), strQuestion, msgBtns, this);
        ...
        QObject::connect(msgBox, &QMessageBox::buttonClicked, this,
                         [=](QAbstractButton *btn)
                         {
                             m_docFilesWatcher->acknowledgeDocumentFileChange(doc);
...
void WidgetMainControl::onGuiDocumentAdded(GuiDocument *guiDoc)
{
    ...
    widgetCtrl->signalMouseMoved.connectSlot(
        [=](int xPos, int yPos)
        {
