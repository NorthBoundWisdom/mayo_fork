...
QVariant GuiDocumentListModel::data(const QModelIndex &index, int role) const
{
    ...
    switch (role)
    {
    case Qt::ToolTipRole: return filepathTo<QString>(filepathCanonical(doc->filePath()));
    case Qt::DisplayRole:
    case Qt::EditRole: return to_QString(doc->name());
    }
...
{
    // NOTE: don't use rowCount() as it's virtual and appendGuiDocument() is
...
void GuiDocumentListModel::onDocumentNameChanged(const DocumentPtr &doc,
                                                 const std::string & /*name*/)
{
    auto itFound =
        std::find_if(m_vecGuiDocument.cbegin(), m_vecGuiDocument.cend(),
