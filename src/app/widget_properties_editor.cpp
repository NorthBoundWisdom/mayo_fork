...
bool WidgetPropertiesEditor::overridePropertyUnitTranslation(const BasePropertyQuantity *prop,
                                                             UnitTranslation unitTr)
{
    return d->itemDelegate->overridePropertyUnitTranslation(prop, unitTr);
}

void WidgetPropertiesEditor::Private::createQtProperty(Property *property,
...
{
    return group && group->treeItem && group->treeItem->treeWidget() == ui->treeWidget_Browser;
}
