    ...
    , m_app(app)
{
    ...
    app->signalDocumentAboutToClose.connectSlot(&DocumentFilesWatcher::onDocumentAboutToClose,
                                                this);
    app->signalDocumentFilePathChanged.connectSlot(&DocumentFilesWatcher::onDocumentFilePathChanged,
                                                   this);
}
...
QFileSystemWatcher *DocumentFilesWatcher::fileSystemWatcher()
{
    if (!m_fileSystemWatcher)
    {
        ...
        QObject::connect(m_fileSystemWatcher, &QFileSystemWatcher::fileChanged, this,
                         &DocumentFilesWatcher::onFileChanged);
    }
...
void DocumentFilesWatcher::onFileChanged(const QString &strFilePath)
{
    if (m_isEnabled)
    {
        ...
        {
            // qDebug() << "onFileChanged()" << strFilePath << " -- file change !ack:
            // signal rejected";
            return;
        }
        ...
        {
            // qDebug() << "onFileChanged()" << strFilePath << " -- being modified:
            // signal rejected";
            return;
        }

        // qDebug() << "onFileChanged()" << strFilePath << " -- register callback:
        // signal accepted";
        ...
        QTimer::singleShot(sendSignalDelay_ms, this,
                           [=]
                           {
                               if (m_setDocBeingChanged.find(doc) != m_setDocBeingChanged.cend())
                               {
                                   // qDebug() << "onFileChanged()" << strFilePath << " -- timer
                                   // callback: signal";
...
{
    auto it =
        std::find(m_vecNonAckDocumentChanged.cbegin(), m_vecNonAckDocumentChanged.cend(), doc);
