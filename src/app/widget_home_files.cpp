...
private:
    void reloadRecentFiles()
    {
        ...
        auto fnToString = [=](const QDateTime &dateTime)
        {
            ...
            else if (diffDays == 1)
            {
                return WidgetHomeFiles::tr("yersterday %1").arg(strTime);
                     ...
                     date.weekNumber() == currentDate.weekNumber())
            {
        ...
        for (const RecentFile &recentFile : listRecentFile)
        {
            ...
            item.description = WidgetHomeFiles::tr("%1\n\n"
                                                   "Size: %2\n\n"
                                                   "Created: %3\n"
                                                   "Modified: %4\n"
                                                   "Read: %5\n")
    ...
    static constexpr const char ImageId_NewDocument[] =
        "NewDocument_beae5f60-78a5-4b4e-8875-2dcebdbb4c58";
    static constexpr const char ImageId_OpenDocuments[] =
        "OpenDocuments_945b1913-59fb-4150-9000-f66332f850fe";
