...
static TreeItemType treeItemType(const QTreeWidgetItem *treeItem)
{
    ...
    return varType.isValid() ? static_cast<Internal::TreeItemType>(varType.toInt()) :
    ...
    , m_ui(new Ui_WidgetModelTree)
{
    ...
    modelTreeBtns->addButton(idBtnRemove, mayoTheme()->icon(Theme::Icon::Cross),
                             tr("Remove from document"));
    modelTreeBtns->setButtonDetection(idBtnRemove, Internal::TreeItemTypeRole,
    ...
    QObject::connect(modelTreeBtns, &ItemViewButtons::buttonClicked, this,
                     [=](int btnId, const QModelIndex &index)
                     {
                         if (btnId == idBtnRemove && index.isValid())
                         {
                             const QTreeWidgetItem *treeItem =
                                 m_ui->treeWidget_Model->itemFromIndex(index);
                             const DocumentTreeNode entityNode =
                                 Internal::treeItemDocumentTreeNode(treeItem);
...
void WidgetModelTree::registerGuiApplication(GuiApplication *guiApp)
{
    ...
    app->signalDocumentEntityAboutToBeDestroyed.connectSlot(
        &WidgetModelTree::onDocumentEntityAboutToBeDestroyed, this);

    m_guiApp->selectionModel()->signalChanged.connectSlot(
        &WidgetModelTree::onApplicationItemSelectionModelChanged, this);
    m_guiApp->signalGuiDocumentAdded.connectSlot(
        [=](GuiDocument *guiDoc)
        {
            guiDoc->signalNodesVisibilityChanged.connectSlot(
                [=](const GuiDocument::MapVisibilityByTreeNodeId &mapNodeId)
...
{
    auto it =
        std::find_if(std::next(m_vecBuilder.cbegin()), m_vecBuilder.cend(),
...
WidgetModelTreeBuilder *WidgetModelTree::findSupportBuilder(const DocumentTreeNode &node) const
{
    ...
    auto it = std::find_if(std::next(m_vecBuilder.cbegin()), m_vecBuilder.cend(),
                           [=](const BuilderPtr &builder)
                           { return builder->supportsDocumentTreeNode(node); });
    return it != m_vecBuilder.cend() ? it->get() : m_vecBuilder.front().get();
}
...
void WidgetModelTree::onDocumentEntityAboutToBeDestroyed(const DocumentPtr &doc,
                                                         TreeNodeId entityId)
{
...
//         this->findSupportBuilder(docItem)->refreshTextTreeItem(docItem,
//         treeItem);
// }

void WidgetModelTree::onTreeWidgetDocumentSelectionChanged(const QItemSelection &selected,
...
void WidgetModelTree::onApplicationItemSelectionModelChanged(Span<const ApplicationItem> selected,
                                                             Span<const ApplicationItem> deselected)
{
...
void WidgetModelTree::connectTreeModelDataChanged(bool on)
{
    if (on)
    {
        m_connTreeModelDataChanged =
            QObject::connect(m_ui->treeWidget_Model->model(), &QAbstractItemModel::dataChanged,
...
void WidgetModelTree::connectTreeWidgetDocumentSelectionChanged(bool on)
{
    if (on)
    {
        m_connTreeWidgetDocumentSelectionChanged = QObject::connect(
            m_ui->treeWidget_Model->selectionModel(), &QItemSelectionModel::selectionChanged, this,
            &WidgetModelTree::onTreeWidgetDocumentSelectionChanged, Qt::UniqueConnection);
    }
    else
    {
...
void WidgetModelTree::onTreeModelDataChanged(const QModelIndex &topLeft,
