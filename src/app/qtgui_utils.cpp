...
QColor lerp(const QColor &a, const QColor &b, double t)
{
    ...
    return QColor(MathUtils::lerp(a.red(), b.red(), t), MathUtils::lerp(a.green(), b.green(), t),
                  MathUtils::lerp(a.blue(), b.blue(), t), MathUtils::lerp(a.alpha(), b.alpha(), t));
}
...
QColor linearColorAt(const QGradient &gradient, double t)
{
    ...
    auto itLower = std::lower_bound(stops.cbegin(), stops.cend(), QGradientStop(t, QColor()),
...
QPixmap toQPixmap(const Image_PixMap &pixmap)
{
    ...
    const QImage img(pixmap.Data(), int(pixmap.Width()), int(pixmap.Height()),
                     int(pixmap.SizeRowBytes()), fnToQImageFormat(pixmap.Format()));
    if (img.isNull())
        return {};
