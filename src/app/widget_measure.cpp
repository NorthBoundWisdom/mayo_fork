...
// Returns the tool object adapted for the graphics object 'gfxObject' and
// measure type Note: the search is performed in the array returned by
...
// Helper function to iterate and execute function 'fn' on all the graphics
// objects owned by a measure display
    ...
    , m_guiDoc(guiDoc)
{
    ...
    m_ui->setupUi(this);
    QObject::connect(m_ui->combo_MeasureType, qOverload<int>(&QComboBox::currentIndexChanged), this,
                     &WidgetMeasure::onMeasureTypeChanged);
    QObject::connect(m_ui->combo_LengthUnit, qOverload<int>(&QComboBox::currentIndexChanged), this,
                     &WidgetMeasure::onMeasureUnitsChanged);
    QObject::connect(m_ui->combo_AngleUnit, qOverload<int>(&QComboBox::currentIndexChanged), this,
                     &WidgetMeasure::onMeasureUnitsChanged);
    QObject::connect(m_ui->combo_AreaUnit, qOverload<int>(&QComboBox::currentIndexChanged), this,
                     &WidgetMeasure::onMeasureUnitsChanged);
    QObject::connect(m_ui->combo_VolumeUnit, qOverload<int>(&QComboBox::currentIndexChanged), this,
                     &WidgetMeasure::onMeasureUnitsChanged);
...
void WidgetMeasure::setMeasureOn(bool on)
{
    ...
    {
        gfxScene->foreachDisplayedObject(
...
void WidgetMeasure::onMeasureTypeChanged(int id)
{
    ...
    // Note: don't call "ui->comboUnit->setVisible(labelUnit->isVisible())"
    // because at this point
    //       QWidget::isVisible() might not be effective(probably needs to process
    //       eventloop)
...
void WidgetMeasure::onMeasureUnitsChanged()
{
    ...
    for (IMeasureDisplayPtr &measure : m_vecMeasureDisplay)
    {
        ...
        foreachGraphicsObject(measure,
                              [](const GraphicsObjectPtr &gfxObject) { gfxObject->Redisplay(); });
    }
...
void WidgetMeasure::onGraphicsSelectionChanged()
{
    ...
    auto fnAddMeasureDisplay =
        [&](std::unique_ptr<IMeasureDisplay> ptr, std::initializer_list<GraphicsOwnerPtr> listOwner)
    {
        if (ptr)
        {
    ...
    // Create MeasureDisplay objects needing a newly single selected graphics
    // object
    ...
    // Create MeasureDisplay objects needing currently two selected graphics
    // objects
    if (vecSelectedOwner_onEntry.size() >= 1)
    {
        for (const GraphicsOwnerPtr &owner : vecNewSelected)
        {
            ...
            const GraphicsOwnerPtr &prevOwner = indexOwner == 0 ? vecSelectedOwner_onEntry.back() :
                                                                  vecNewSelected.at(indexOwner - 1);
            try
            {
                const MeasureValue value =
                    IMeasureTool_computeValue(*m_tool, measureType, prevOwner, owner);
                if (MeasureValue_isValid(value))
                    fnAddMeasureDisplay(BaseMeasureDisplay::createFrom(measureType, value),
                                        {prevOwner, owner});
            }
            catch (const IMeasureError &err)
            {
    ...
    for (IMeasureDisplayPtr &measure : vecNewMeasureDisplay)
    {
        ...
        foreachGraphicsObject(
            measure, [=](const GraphicsObjectPtr &gfxObject)
...
void WidgetMeasure::updateMessagePanel()
{
    ...
    if (!m_errorMessage.isEmpty() || m_vecMeasureDisplay.empty())
    {
        ...
        const auto msgTextColorRole = m_errorMessage.isEmpty() ?
                                          Theme::Color::MessageIndicator_InfoText :
                                          Theme::Color::MessageIndicator_ErrorText;
        const auto msgBackgroundColorRole = m_errorMessage.isEmpty() ?
        ...
        const QString msg =
            m_errorMessage.isEmpty() ? tr("Select entities to measure") : m_errorMessage;
    ...
    else
    {
        ...
        // Helper function to add the text of an IMeasureDisplay object in the
        // message panel
        ...
        // Handle the case where there are many measures and sum is a supported
        // operation
...
void WidgetMeasure::eraseMeasureDisplay(const IMeasureDisplay *measure)
{
    ...
    auto it = std::find_if(m_vecMeasureDisplay.begin(), m_vecMeasureDisplay.end(),
...
void WidgetMeasure::eraseLink(const GraphicsOwner_MeasureDisplay *link)
{
    ...
    m_vecLinkGfxOwnerMeasure.erase(m_vecLinkGfxOwnerMeasure.begin() +
                                   (link - &m_vecLinkGfxOwnerMeasure.front()));
}

const WidgetMeasure::GraphicsOwner_MeasureDisplay *
WidgetMeasure::findLink(const GraphicsOwnerPtr &owner) const
{
    auto itFound = std::find_if(m_vecLinkGfxOwnerMeasure.begin(), m_vecLinkGfxOwnerMeasure.end(),
                                [=](const GraphicsOwner_MeasureDisplay &link)
                                { return link.gfxOwner == owner; });
