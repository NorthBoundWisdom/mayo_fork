...
// Specific role for storing the SettingNodeId value of an item node in the
// Settings tree
...
// Provides a custom style for deactivating mouse wheel interaction with combo
// boxes
...
// Reserved name for Property(ie setting) editors. QObject::objectName() will
// return this name
    ...
    , m_settings(settings)
{
    ...
    // Add action "Restore defaults" for group/section items in "left-side" tree
    // view
    {
        auto treeViewBtns = new ItemViewButtons(m_ui->treeView_GroupSections, this);
        constexpr int idBtnRestore = 1;
        treeViewBtns->addButton(idBtnRestore, mayoTheme()->icon(Theme::Icon::Reload),
        ...
        QObject::connect(treeViewBtns, &ItemViewButtons::buttonClicked, this,
    ...
    const QFont fontItemGroupSection =
        QtGuiUtils::FontChange(m_ui->listWidget_Settings->font()).bold(true).scalePointSizeF(1.7);
        ...
        for (int iSection = 0; iSection < settings->sectionCount(indexGroup); ++iSection)
        {
            ...
            if (!settings->isDefaultGroupSection(indexSection))
            {
                const QString titleSection =
                    tr("%1 / %2").arg(titleGroup, to_QString(settings->sectionTitle(indexSection)));
            ...
            {
                Property *property =
                    settings->property(Settings::SettingIndex(indexSection, iSetting));
    ...
    // Enable/disable editor widget when the corresponding setting status is
    // changed
    m_connSettingsEnabled = settings->signalEnabled.connectSlot(
        [=](const Property *setting, bool on)
        {
    ...
    // Backup initial value of changed settings, so they can be restored on dialog
    // cancellation
    m_connSettingsAboutToChange = settings->signalAboutToChange.connectSlot(
        [=](Property *property)
        {
            if (m_mapSettingInitialValue.find(property) == m_mapSettingInitialValue.cend())
            {
                const Settings::Variant propertyValue =
                    settings->propertyValueConversion().toVariant(*property);
                m_mapSettingInitialValue.insert({property, propertyValue});
            }
        });

    // Synchronize editor widget when value of the corresponding property is
    // changed
    m_connSettingsChanged = settings->signalChanged.connectSlot(
    ...
    // When a setting is clicked in the "right-side" view then scroll to and
    // select corresponding tree item in the "left-side" view
    QObject::connect(m_ui->listWidget_Settings, &QListWidget::currentRowChanged, this,
                     [=](int listRow)
                     {
                         auto listItem = m_ui->listWidget_Settings->item(listRow);
                         const QVariant variantNodeId =
                             listItem ? listItem->data(ItemSettingNodeId_Role) : QVariant();
                         ...
                         {
                             m_ui->treeView_GroupSections->scrollTo(
                                 indexList.front(), QAbstractItemView::PositionAtTop);
    ...
    // When a group/section is clicked in the "left-side" view then scroll to the
    // list item in the "right-side" view
    QObject::connect(
        m_ui->treeView_GroupSections->selectionModel(), &QItemSelectionModel::currentChanged, this,
        [=](const QModelIndex &current)
        {
            ...
            if (!indexList.isEmpty())
                m_ui->listWidget_Settings->scrollTo(indexList.front(),
                                                    QAbstractItemView::PositionAtTop);
        });

    // Action for "Cancel" button : restore changed properties to their initial
    // values
    QObject::connect(m_ui->buttonBox, &QDialogButtonBox::rejected, this,
                     &DialogOptions::cancelChanges);

    // Action for "Restore defaults" button
    auto btnRestoreDefaults = m_ui->buttonBox->button(QDialogButtonBox::RestoreDefaults);
    QObject::connect(btnRestoreDefaults, &QPushButton::clicked, this,
                     [=] { settings->resetAll(); });
...
QWidget *DialogOptions::createEditor(Property *property, QWidget *parentWidget) const
{
    ...
    widgetLayout->addWidget(panelEditor);
    {
        ...
        if (editor)
        {
            ...
            {
                if (qobject_cast<QComboBox *>(childWidget) ||
                    qobject_cast<QLineEdit *>(childWidget))
...
void DialogOptions::loadFromFile()
{
    ...
    const QString filepath = QFileDialog::getOpenFileName(this, tr("Choose INI file"), startDirPath,
                                                          tr("INI files(*.ini)"));
    if (filepath.isEmpty())
        return;
    ...
    if (!fi.exists())
    {
        QtWidgetsUtils::asyncMsgBoxCritical(this, tr("Error"),
                                            tr("'%1' doesn't exist").arg(filepath));
        return;
    }
    ...
    {
        QtWidgetsUtils::asyncMsgBoxCritical(this, tr("Error"),
                                            tr("'%1' is not readable").arg(filepath));
        return;
    }
...
{
    const QString startDirPath = QString();
    const QString filepath = QFileDialog::getSaveFileName(this, tr("Choose INI file"), startDirPath,
                                                          tr("INI files(*.ini)"));
    if (filepath.isEmpty())
        return;
    ...
    if (fileSettings.get().status() != QSettings::NoError)
        QtWidgetsUtils::asyncMsgBoxCritical(this, tr("Error"),
                                            tr("Error when writing to '%1'").arg(filepath));
}
...
void DialogOptions::handleTreeViewButtonClick_restoreDefaults(const QModelIndex &index)
{
    ...
    if (isGroup)
    {
        ...
        const bool defaultSectionNotEmpty = m_settings->isDefaultGroupSection(firstSectionIndex) &&
