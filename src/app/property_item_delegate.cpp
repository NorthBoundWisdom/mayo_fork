...
void PropertyItemDelegate::setPropertyEditorFactory(
    std::unique_ptr<IPropertyEditorFactory> editorFactory)
{
...
void PropertyItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
                                 const QModelIndex &index) const
{
    ...
    if (index.column() == 1)
    {
        ...
        if (prop && prop->dynTypeName() == PropertyOccColor::TypeName)
        {
            ...
            const QPixmap pixColor =
                IPropertyEditorFactory::colorSquarePixmap(color, option.rect.height());
            ...
            QApplication::style()->drawItemText(
                painter, labelRect, Qt::AlignLeft | Qt::AlignVCenter, option.palette,
                option.state.testFlag(QStyle::State_Enabled), strColor);
    ...
    {
        const Property *prop = qvariant_cast<Property *>(index.data());
        if (!prop->isUserReadOnly() && option.state.testFlag(QStyle::State_Enabled) &&
            option.state.testFlag(QStyle::State_MouseOver))
        {
            ...
            painter->drawPixmap(option.rect.x() + itemSize.width() + 4,
...
QWidget *PropertyItemDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &,
                                            const QModelIndex &index) const
{
    if (index.column() == 0)
        return nullptr;
