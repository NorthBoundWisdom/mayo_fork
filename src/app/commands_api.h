...
public:
    ...
    virtual bool getEnabledStatus() const
    {
        return true;
    }
...
protected:
    ...
    GuiApplication *guiApp() const
    {
        return m_context ? m_context->guiApp() : nullptr;
    }
    TaskManager *taskMgr() const
    {
        return m_context ? m_context->taskMgr() : nullptr;
    }
    QWidget *widgetMain() const
    {
        return m_context ? m_context->widgetMain() : nullptr;
    }
    Document::Identifier currentDocument() const
    {
        return m_context ? m_context->currentDocument() : -1;
    }
...
// Each command in the container is mapped to a unique identifier(ie the
// "command name")
...
public:
    ...
    void foreachCommand(Function fn);

    // Returns the Command object mapped to 'name'
    // That object was previously created and associated with a call to
    // addCommand()/addNamedCommand() Might return null in case no command is
    // mapped to 'name'
    Command *findCommand(std::string_view name) const;

    // Helper function to retrieve the action provided by the Command object
    // mapped to 'name' Might return null in case no command is mapped to 'name'
    QAction *findCommandAction(std::string_view name) const;

    // Construct and add new Command object with arguments 'args'
    // The command is associated to identigfier 'name' and can be retrieved later
    // on with findCommand()
    template <typename CmdType, typename... Args>
    CmdType *addCommand(std::string_view name, Args... p);

    // Same behavior as addCommand() function
    // The command name is implicit and found by assuming the presence of
    // CmdType::Name class member
    template <typename CmdType, typename... Args>
    CmdType *addNamedCommand(Args... p);
