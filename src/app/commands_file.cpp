...
QString fileFilter(IO::Format format)
{
    ...
    return Command::tr("%1 files(%2)").arg(to_QString(IO::formatIdentifier(format))).arg(filter);
}
...
struct ImportExportSettings
{
    ...
    {
        return {AppModule::get()->properties()->lastOpenDir.value(),
    ...
    static void save(const ImportExportSettings &sets)
    {
        ...
        AppModule::get()->properties()->lastSelectedFormatFilter.setValue(
            to_stdString(sets.selectedFilter));
    }
};
...
struct OpenFileNames
{
                             ...
                             OpenFileNames::GetOption option = OpenFileNames::GetMany)
    {
        ...
        {
            const QStringList listStrFilePath = QFileDialog::getOpenFileNames(
                parentWidget, dlgTitle, dlgOpenDir, dlgFilter, dlgPtrSelFilter);
            result.listFilepath.clear();
        ...
        if (!result.listFilepath.empty())
        {
            ...
            result.selectedFormat = result.lastIoSettings.selectedFilter != allFilesFilter ?
...
void FileCommandTools::openDocumentsFromList(IAppContext *context,
                                             Span<const FilePath> listFilePath)
{
    ...
    for (const FilePath &fp : listFilePath)
    {
        ...
        if (docPtr.IsNull())
        {
            ...
            // will be released on next task creation, so the document won't be
            // destroyed
            const Document::Identifier newDocId = docPtr->identifier();
            const TaskId taskId = context->taskMgr()->newTask(
                [=](TaskProgress *progress)
                {
                    ...
                    const bool okImport =
                        appModule->ioSystem()
                            ...
                            .withEntityPostProcess(
                                [=](TDF_Label labelEntity, TaskProgress *progress)
                                { appModule->computeBRepMesh(labelEntity, progress); })
                            .withEntityPostProcessRequiredIf(&IO::formatProvidesBRep)
                            .withEntityPostProcessInfoProgress(
                                20, Command::textIdTr("Mesh BRep shapes"))
...
void FileCommandTools::importInDocument(IAppContext *context, const DocumentPtr &targetDoc,
                                        Span<const FilePath> listFilePaths)
{
    ...
    const QString taskTitle = listFilePaths.size() > 1 ?
                                  Command::tr("Import") :
    ...
    : CommandRecentFiles(context)
{
    QObject::connect(containerMenu, &QMenu::aboutToShow, this,
                     &CommandRecentFiles::recreateEntries);
}
...
void CommandImportInCurrentDocument::execute()
{
    ...
    FileCommandTools::importInDocument(this->context(), guiDoc->document(),
                                       resFileNames.listFilepath);
...
void CommandExportSelectedApplicationItems::execute()
{
    ...
    const QString strFilepath = QFileDialog::getSaveFileName(
        this->widgetMain(), Command::tr("Select Output File"),
        filepathTo<QString>(lastSettings.openDir), listWriterFileFilter.join(QLatin1String(";;")),
    ...
    const TaskId taskId = this->taskMgr()->newTask(
        [=](TaskProgress *progress)
        {
            QElapsedTimer chrono;
            chrono.start();
            const bool okExport = appModule->ioSystem()
                                      ->exportApplicationItems()
            ...
            if (okExport)
                appModule->emitInfo(
                    fmt::format(Command::textIdTr("Export time: {}ms"), chrono.elapsed()));
        });
    ...
    : Command(context)
{
    ...
    QObject::connect(context, &IAppContext::currentDocumentChanged, this,
                     &CommandCloseCurrentDocument::updateActionText);
    this->app()->signalDocumentNameChanged.connectSlot(
...
void CommandCloseCurrentDocument::updateActionText(Document::Identifier docId)
{
    ...
    const QString textActionClose =
        docPtr ? Command::tr("Close %1").arg(strFilepathQuoted(docName)) : Command::tr("Close");
    ...
    : Command(context)
{
    ...
    QObject::connect(context, &IAppContext::currentDocumentChanged, this,
                     &CommandCloseAllDocumentsExceptCurrent::updateActionText);
    this->app()->signalDocumentNameChanged.connectSlot(
        [=](const DocumentPtr &doc)
        {
            if (this->currentDocument() == doc->identifier())
...
void CommandCloseAllDocumentsExceptCurrent::updateActionText(Document::Identifier docId)
{
    ...
    const QString docName = to_QString(docPtr ? docPtr->name() : std::string{});
    const QString textActionClose =
        docPtr ? Command::tr("Close all except %1").arg(strFilepathQuoted(docName)) :
