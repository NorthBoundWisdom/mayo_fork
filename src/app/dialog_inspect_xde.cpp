...
static void loadLabelAttributes(const TDF_Label &label, QTreeWidgetItem *treeItem)
{
    for (TDF_AttributeIterator it(label); it.More(); it.Next())
    {
        ...
        else
        {
            ...
            int i = 0;
            while (i < strDump.size() && strDump.at(i).isSpace())
                ++i;
            ...
            text = wordStart < strDump.size() ? strDump.mid(wordStart, wordEnd - wordStart) :
                                                QStringLiteral("??");
...
createPropertyTreeItem(const QString &text, double value,
                       const QStringUtils::TextOptions &options = appDefaultTextOptions())
{
    auto itemProperty = createPropertyTreeItem(text);
...
static QTreeWidgetItem *createPropertyTreeItem(const QString &text,
                                               const OccHandle<TCollection_HAsciiString> &value)
{
...
{
    auto itemPnt = createPropertyTreeItem(text);
    const QString textAx2 = QString("Location %1 - Direction %2")
...
static void loadLabelMaterialProperties(const TDF_Label &label,
                                        const OccHandle<XCAFDoc_MaterialTool> &materialTool,
...
static QPixmap loadPixmap(const QByteArray &fileData)
{
    ...
    if (!okLoad || pixmap.isNull())
    {
        // QPixmap::loadFromData() failed, try with OpenCascade
        // Image_AlienPixMap::Load()
        Image_AlienPixMap occPixmap;
        okLoad = occPixmap.Load(reinterpret_cast<const Standard_Byte *>(fileData.constData()),
                                fileData.size(), TCollection_AsciiString{});
        if (okLoad)
            pixmap = QtGuiUtils::toQPixmap(occPixmap);
    }
...
// Provides a QTreeWidgetItem specialized to display an image file with a
// tooltip QTreeWidgetItem::setToolTip() could be used but it forces all image
// files to be loaded on tree item construction This helper allows "lazy"
    ...
    QVariant data(int column, int role) const override
    {
        ...
        if (ptrItem->strToolTip.isEmpty())
        {
            ...
            const QPixmap pixmap =
                isFilePathDefined ? loadPixmap(ptrItem->filePath) : loadPixmap(ptrItem->fileData);
            const uintmax_t imageSize =
                isFilePathDefined ? filepathFileSize(ptrItem->filePath) : ptrItem->fileData.size();
            if (!pixmap.isNull())
            {
                QBuffer bufferPixmap;
                const int pixmapWidth =
                    std::min(pixmap.width(), int(400 * qGuiApp->devicePixelRatio()));
                const QPixmap pixmapClamped = pixmap.scaledToWidth(pixmapWidth);
                pixmapClamped.save(&bufferPixmap, "PNG");
                const QString strImageSize =
                    QStringUtils::bytesText(imageSize, appDefaultTextOptions().locale);
                ptrItem->strToolTip =
                    QString("<img src=\"data:image/png;base64,%1\" width=\"%2\" "
                            "height=\"%3\"><p>%4</p>")
            ...
            {
                ptrItem->strToolTip =
                    DialogInspectXde::tr("Error when loading texture file(invalid path?)");
            }
        }
...
static QTreeWidgetItem *createPropertyTreeItem(const QString &text,
                                               const OccHandle<Image_Texture> &imgTexture)
{
    ...
    else if (imgTexture->DataBuffer() && !imgTexture->DataBuffer()->IsEmpty())
    {
        ...
        const int buffSize = CppUtils::safeStaticCast<int>(imgTexture->DataBuffer()->Size());
...
static void
loadLabelVisMaterialProperties(const TDF_Label &label,
                               ...
                               QTreeWidgetItem *treeItem)
{
    ...
    fnAddItem(
        createPropertyTreeItem("IsSetShapeMaterial", visMaterialTool->IsSetShapeMaterial(label)));

    auto fnCreateVisMaterialTreeItem =
        [](const QString &text, const OccHandle<XCAFDoc_VisMaterial> &material)
    {
        ...
        item->addChild(
            createPropertyTreeItem("FaceCulling", MetaEnum::name(material->FaceCulling())));
#else
        ...
        if (material->HasPbrMaterial())
        {
            ...
            itemPbr->addChild(
                createPropertyTreeItem("BaseColorTexture", pbrMaterial.BaseColorTexture));
            itemPbr->addChild(createPropertyTreeItem("MetallicRoughnessTexture",
                                                     pbrMaterial.MetallicRoughnessTexture));
            itemPbr->addChild(
                createPropertyTreeItem("EmissiveTexture", pbrMaterial.EmissiveTexture));
            itemPbr->addChild(
                createPropertyTreeItem("OcclusionTexture", pbrMaterial.OcclusionTexture));
            ...
            itemPbr->addChild(
                createPropertyTreeItem("RefractionIndex", pbrMaterial.RefractionIndex));
        ...
        if (material->HasCommonMaterial())
        {
            ...
            itemCommon->addChild(
                createPropertyTreeItem("DiffuseTexture", commonMaterial.DiffuseTexture));
            itemCommon->addChild(
                createPropertyTreeItem("AmbientColor", commonMaterial.AmbientColor));
            itemCommon->addChild(
                createPropertyTreeItem("DiffuseColor", commonMaterial.DiffuseColor));
            itemCommon->addChild(
                createPropertyTreeItem("SpecularColor", commonMaterial.SpecularColor));
            itemCommon->addChild(
                createPropertyTreeItem("EmissiveColor", commonMaterial.EmissiveColor));
            itemCommon->addChild(createPropertyTreeItem("Shininess", commonMaterial.Shininess));
            itemCommon->addChild(
                createPropertyTreeItem("Transparency", commonMaterial.Transparency));
...
static void loadLabelColorProperties(const TDF_Label &label,
                                     ...
                                     QTreeWidgetItem *treeItem)
{
    ...
    fnAddItem(
        createPropertyTreeItem("IsSet_ColorSurf", colorTool->IsSet(label, XCAFDoc_ColorSurf)));
    fnAddItem(
        createPropertyTreeItem("IsSet_ColorCurv", colorTool->IsSet(label, XCAFDoc_ColorCurv)));
                                     ...
                                     QTreeWidgetItem *treeItem)
{
    ...
    if (XCAFDoc_ShapeTool::IsReference(label))
    {
        ...
        {
            const QString textItemRefShape = to_QString(CafUtils::labelTag(labelRef)) + " " +
...
static void loadLabelDimensionProperties(const TDF_Label &label, QTreeWidgetItem *treeItem)
{
    ...
    fnAddItem(createPropertyTreeItem("IsDimWithPlusMinusTolerance",
                                     dimObject->IsDimWithPlusMinusTolerance()));
    ...
    fnAddItem(createPropertyTreeItem("IsDimWithClassOfTolerance",
                                     dimObject->IsDimWithClassOfTolerance()));
    if (dimObject->IsDimWithClassOfTolerance())
    {
        ...
        if (dimObject->GetClassOfTolerance(hole, formVariance, grade))
        {
            ...
            fnAddChildItem(createPropertyTreeItem(
                "DimensionFormVariance",
                MetaEnum::nameWithoutPrefix(formVariance, "XCAFDimTolObjects_")));
            fnAddChildItem(createPropertyTreeItem(
                "DimensionGrade", MetaEnum::nameWithoutPrefix(grade, "XCAFDimTolObjects_")));
        }
    }
    ...
    if (dimObject->HasDescriptions())
    {
        for (int i = 1; i <= dimObject->NbDescriptions(); ++i)
            fnAddChildItem(
                createPropertyTreeItem("DescriptionName", dimObject->GetDescriptionName(i)));
    }
...
static void loadLabelDatumProperties(const TDF_Label &label, QTreeWidgetItem *treeItem)
{
    ...
    fnAddItem(createPropertyTreeItem("Name", datumObject->GetName()));
    {
        ...
        fnAddItem(createPropertyTreeItem(
            "Modifier", MetaEnum::nameWithoutPrefix(modfValue, "XCAFDimTolObjects_")));
    ...
    {
        fnAddChildItem(
            createPropertyTreeItem("DatumTargetAxis", datumObject->GetDatumTargetAxis()));
        fnAddChildItem(
            createPropertyTreeItem("DatumTargetLength", datumObject->GetDatumTargetLength()));
        fnAddChildItem(
            createPropertyTreeItem("DatumTargetWidth", datumObject->GetDatumTargetWidth()));
        fnAddChildItem(
            createPropertyTreeItem("DatumTargetNumber", datumObject->GetDatumTargetNumber()));
    }
    ...
    if (datumObject->HasPointText())
        fnAddChildItem(
            createPropertyTreeItem("PointTextAttach", datumObject->GetPointTextAttach()));
...
static void loadLabelGeomToleranceProperties(const TDF_Label &label, QTreeWidgetItem *treeItem)
{
    ...
    fnAddItem(createPropertyTreeItem(
        "TypeOfValue",
        MetaEnum::nameWithoutPrefix(tolObject->GetTypeOfValue(), "XCAFDimTolObjects_")));
    fnAddItem(createPropertyTreeItem("Value", tolObject->GetValue()));
    fnAddItem(createPropertyTreeItem(
        "MaterialRequirementModifier",
        MetaEnum::nameWithoutPrefix(tolObject->GetMaterialRequirementModifier(),
                                    "XCAFDimTolObjects_")));
    fnAddItem(createPropertyTreeItem(
        "ZoneModifier",
    ...
    if (tolObject->HasAffectedPlane())
    {
        ...
        fnAddChildItem(createPropertyTreeItem("AffectedPlaneType",
                                              tolObject->GetAffectedPlane().Position().Ax2()));
    }
    ...
    , m_ui(new Ui_DialogInspectXde)
{
    ...
    QObject::connect(m_ui->treeWidget_Document, &QTreeWidget::itemClicked, this,
                     &DialogInspectXde::onLabelTreeWidgetItemClicked);
}
...
void DialogInspectXde::load(const OccHandle<TDocStd_Document> &doc)
{
    ...
    {
        QtWidgetsUtils::asyncMsgBoxCritical(this, tr("Error"),
                                            tr("This document is not suitable for XDE"));
        return;
    }
...
void DialogInspectXde::onLabelTreeWidgetItemClicked(QTreeWidgetItem *item, int /*column*/)
{
    ...
    if (varLabel.isValid())
    {
        ...
        if (isShapeLabel)
            Internal::loadLabelShapeProperties(label, shapeTool,
                                               fnCreateLabelTreeItem(tr("Shape")));

        auto colorTool = XCAFDoc_DocumentTool::ColorTool(m_doc->Main());
        if (colorTool->IsColor(label) || isShapeLabel)
            Internal::loadLabelColorProperties(label, colorTool,
                                               fnCreateLabelTreeItem(tr("Color")));

        auto materialTool = XCAFDoc_DocumentTool::MaterialTool(m_doc->Main());
        if (materialTool->IsMaterial(label) || isShapeLabel)
            Internal::loadLabelMaterialProperties(label, materialTool,
                                                  fnCreateLabelTreeItem(tr("Material")));

#if OCC_VERSION_HEX >= OCC_VERSION_CHECK(7, 5, 0)
        auto visMaterialTool = XCAFDoc_DocumentTool::VisMaterialTool(m_doc->Main());
        if (visMaterialTool->IsMaterial(label) || isShapeLabel)
            Internal::loadLabelVisMaterialProperties(label, visMaterialTool,
                                                     fnCreateLabelTreeItem(tr("VisMaterial")));
#endif
        ...
        if (pmiTool->IsGeomTolerance(label))
            Internal::loadLabelGeomToleranceProperties(label,
                                                       fnCreateLabelTreeItem(tr("GeomTolerance")));
    }
