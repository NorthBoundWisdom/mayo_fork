    ...
    , m_view(view)
{
    ...
    m_vecClipPlaneData = {{new Graphic3d_ClipPlane(gp_Pln(gp::Origin(), gp::DX())),
...
void WidgetClipPlanes::connectUi(ClipPlaneData *data)
{
    ...
    QObject::connect(ui.check_On, &QAbstractButton::toggled, this,
                     [=](bool on)
                     {
    ...
    QObject::connect(posSpin, signalSpinValueChanged, this,
                     [=](double pos)
                     {
    ...
    auto fnConnectDirSpin = [=](QDoubleSpinBox *dirSpin)
    {
        QObject::connect(dirSpin, signalSpinValueChanged, this,
                         [=]
                         {
                             const gp_Vec vecNormal(customXDirSpin->value(),
...
#else
    // TODO Copy the image resource to a temporary file and call
    // Image_AlienPixMap::Load(tempFilePath)
#endif
}
...
double WidgetClipPlanes::UiClipPlane::spinValueToSliderValue(double val) const
{
    ...
    return MathUtils::mappedValue(val, spin->minimum(), spin->maximum(), slider->minimum(),
                                  slider->maximum());
}
...
double WidgetClipPlanes::UiClipPlane::sliderValueToSpinValue(double val) const
{
    const QDoubleSpinBox *spin = this->posSpin();
    const QAbstractSlider *slider = this->posSlider();
    return MathUtils::mappedValue(val, slider->minimum(), slider->maximum(), spin->minimum(),
                                  spin->maximum());
}
