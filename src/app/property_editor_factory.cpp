...
class PropertyEditorI18N
{
    Q_DECLARE_TR_FUNCTIONS(Mayo::PropertyEditorI18N)
};
...
struct PropertyIntEditor : public InterfacePropertyEditor, public QSpinBox
{
        ...
        , m_property(property)
    {
        ...
        QObject::connect(this, qOverload<int>(&QSpinBox::valueChanged),
                         [=](int val) { property->setValue(val); });
    }
...
struct PropertyDoubleEditor : public InterfacePropertyEditor, public QDoubleSpinBox
{
        ...
        , m_property(property)
    {
        ...
        QObject::connect(this, qOverload<double>(&QDoubleSpinBox::valueChanged),
                         [=](double val) { property->setValue(val); });
    }
...
struct PropertyStringEditor : public InterfacePropertyEditor, public QLineEdit
{
        ...
        , m_property(property)
    {
        QObject::connect(this, &QLineEdit::textChanged,
                         [=](const QString &text) { property->setValue(to_stdString(text)); });
    }
...
struct PropertyEnumerationEditor : public InterfacePropertyEditor, public QComboBox
{
        ...
        , m_property(property)
    {
        ...
        QObject::connect(this, qOverload<int>(&QComboBox::activated),
                         [=](int index) { property->setValue(this->itemData(index).toInt()); });
    }
        ...
        , m_property(property)
    {
        ...
        QObject::connect(btnColor, &QAbstractButton::clicked,
                         [=]
                         {
                             ...
                             QObject::connect(dlg, &QColorDialog::colorSelected,
                                              [=](const QColor &c)
                                              {
                                                  property->setValue(
                                                      QtGuiUtils::toColor<Quantity_Color>(c));
...
// Helper generic interface over a XYZ-value property type to get/set the
// coordinates with gp_XYZ
template <typename PropertyCoordsType>
class IProperty3dCoords
{
    // Expected functions
    //     static const gp_XYZ& coords(const PROPERTY_COORDS_TYPE* prop);
    //     static void setCoords(PROPERTY_COORDS_TYPE* prop, const gp_XYZ&
    //     coords);
};

// Partial specialization of IProperty3dCoords for PropertyOccPnt
template <>
struct IProperty3dCoords<PropertyOccPnt>
{
    static const gp_XYZ &coords(const PropertyOccPnt *prop)
    {
        return prop->value().XYZ();
    }
    static void setCoords(PropertyOccPnt *prop, const gp_XYZ &coords)
    {
        prop->setValue(coords);
    }
};

// Partial specialization of IProperty3dCoords for PropertyOccVec
template <>
struct IProperty3dCoords<PropertyOccVec>
{
    static const gp_XYZ &coords(const PropertyOccVec *prop)
    {
        return prop->value().XYZ();
    }
    static void setCoords(PropertyOccVec *prop, const gp_XYZ &coords)
    {
        prop->setValue(coords);
    }
};
...
struct Property3dCoordsEditor : public InterfacePropertyEditor, public QWidget
{
    ...
    void syncWithProperty() override
    {
        auto fnSetEditorCoord = [](QDoubleSpinBox *editor, double coord)
        {
            ...
            auto trRes = UnitSystem::translate(appModule->properties()->unitSystemSchema,
                                               coord * Quantity_Millimeter);
    ...
    static QDoubleSpinBox *createCoordEditor(QWidget *parentWidget, PropertyCoordsType *property,
                                             Coord specCoord)
    {
        auto editor = new QDoubleSpinBox(parentWidget);
        auto appModule = AppModule::get();
        auto trRes =
            UnitSystem::translate(appModule->properties()->unitSystemSchema, 1., Unit::Length);
        ...
        QObject::connect(editor, qOverload<double>(&QDoubleSpinBox::valueChanged),
                         [=](double value)
                         {
...
struct PropertyQuantityEditor : public InterfacePropertyEditor, public QDoubleSpinBox
{
        ...
        , m_property(property)
    {
        ...
        this->setDecimals(AppModule::get()->properties()->unitSystemDecimals.value());
        const double rangeMin = property->constraintsEnabled() ? property->minimum() :
                                                                 std::numeric_limits<double>::min();
        const double rangeMax = property->constraintsEnabled() ? property->maximum() :
...
QWidget *DefaultPropertyEditorFactory::createEditor(Property *property, QWidget *parentWidget) const
{
    ...
    if (propTypeName == PropertyCheckState::TypeName)
        editor =
            new PropertyCheckStateEditor(static_cast<PropertyCheckState *>(property), parentWidget);
    ...
    if (propTypeName == PropertyOccColor::TypeName)
        editor =
            new PropertyOccColorEditor(static_cast<PropertyOccColor *>(property), parentWidget);

    if (propTypeName == PropertyOccPnt::TypeName)
        editor = new Property3dCoordsEditor<PropertyOccPnt>(static_cast<PropertyOccPnt *>(property),
                                                            parentWidget);

    if (propTypeName == PropertyOccVec::TypeName)
        editor = new Property3dCoordsEditor<PropertyOccVec>(static_cast<PropertyOccVec *>(property),
                                                            parentWidget);

    if (propTypeName == PropertyEnumeration::TypeName)
        editor = new PropertyEnumerationEditor(static_cast<PropertyEnumeration *>(property),
                                               parentWidget);

    if (propTypeName == BasePropertyQuantity::TypeName)
        editor =
            new PropertyQuantityEditor(static_cast<BasePropertyQuantity *>(property), parentWidget);
...
void DefaultPropertyEditorFactory::syncEditorWithProperty(QWidget *editor) const
{
    ...
    if (intf)
    {
        QSignalBlocker sigBlocker(editor);
        Q_UNUSED(sigBlocker);
        intf->syncWithProperty();
    }
}

UnitSystem::TranslateResult
IPropertyEditorFactory::unitTranslate(const BasePropertyQuantity *property)
{
    if (!property)
        return {};
    ...
    return UnitSystem::translate(AppModule::get()->properties()->unitSystemSchema,
                                 property->quantityValue(), property->quantityUnit());
}
