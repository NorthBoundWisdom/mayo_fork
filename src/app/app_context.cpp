    ...
    , m_wnd(wnd)
{
    ...
    QObject::connect(m_wnd->widgetPageDocuments(), &WidgetMainControl::currentDocumentIndexChanged,
...
{
    int index = -1;
    auto widgetDoc = this->findWidgetGuiDocument(
        [&](WidgetGuiDocument *candidate)
        {
            ++index;
...
{
    auto widgetDoc = this->findWidgetGuiDocument(
        [=](WidgetGuiDocument *widgetDoc) { return widgetDoc->documentIdentifier() == docId; });
...
WidgetGuiDocument *
AppContext::findWidgetGuiDocument(std::function<bool(WidgetGuiDocument *)> fn) const
{
