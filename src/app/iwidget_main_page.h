...
// Such widgets are called "main pages" or just "pages" and only a single one is
// active at a time
...
public:
    ...
    // Completes initialization of the page(eg by linking some buttons to existing
    // commands)
    virtual void initialize(const CommandContainer *cmdContainer) = 0;

    // Update the activation("enabled" status) of the controls(ie any widget)
    // belonging to this page
    virtual void updatePageControlsActivation() = 0;

signals:
    // Signal emitted when a "global" or "complete" activation at the whole
    // application level is required by this page This might, for example, be
    // consecutive to event internal to this page
