    ...
    , m_settings(settings)
{
    ...
    const auto sectionId_graphicsClipPlanes =
        settings->addSection(groupId_graphics, textId("clipPlanes"));
    const auto sectionId_graphicsMeshDefaults =
        settings->addSection(groupId_graphics, textId("meshDefaults"));
    ...
    // Application
    this->actionOnDocumentFileChange.mutableEnumeration().changeTrContext(
        AppModuleProperties::textIdContext());
    ...
    settings->addResetFunction(groupId_meshing,
                               [&]
                               {
                                   ...
                                   this->meshingChordalDeflection.setQuantity(1 *
                                                                              Quantity_Millimeter);
...
void AppModuleProperties::IO_bindParameters(const IO::System *ioSystem)
{
    ...
    {
        auto sectionId_format =
            m_settings->addSection(groupId_Import, IO::formatIdentifier(format));
    ...
    {
        auto sectionId_format =
            m_settings->addSection(groupId_Export, IO::formatIdentifier(format));
...
{
    // System
    this->unitSystemSchema.mutableEnumeration().changeTrContext(
        AppModuleProperties::textIdContext());

    // Application
    this->language.setDescription(
        textIdTr("Language used for the application. Change will take effect "
                 "after application restart"));
    const auto &enumActionOnDocumentFileChange = this->actionOnDocumentFileChange.enumeration();
    this->actionOnDocumentFileChange.setDescription(fmt::format(
        textIdTr("Action to be done after some opened document file is "
                 "changed(modified) externally\n\n"
                 "Select options `{0}` or `{1}` so the application monitors "
                 "changes made to opened files\n\n"
                 "When such a change is detected then the application proposes "
                 "to reload(open again) the document\n\n"
                 "Select `{1}` to automatically reload documents without any "
                 "user interaction"),
        enumActionOnDocumentFileChange
            .findItemByValue(ActionOnDocumentFileChange::ReloadIfUserConfirm)
            ->name.tr(),
        enumActionOnDocumentFileChange.findItemByValue(ActionOnDocumentFileChange::ReloadSilently)
            ->name.tr()));
    this->linkWithDocumentSelector.setDescription(
        textIdTr("In case where multiple documents are opened, make sure the document "
                 "displayed in "
    ...
    this->forceOpenGlFallbackWidget.setDescription(
        textIdTr("Force usage of the fallback Qt widget to display OpenGL graphics.\n\n"
                 "When `OFF` the application will try to use OpenGL framebuffer for "
                 "rendering, "
                 "this allows to display overlay widgets(eg measure tools panel) with "
                 "translucid "
                 "background. "
                 "However using OpenGL framebuffer might cause troubles for some users(eg "
                 "empty "
                 "3D window) especially on macOS.\n\n"
                 "When `ON` the application will use a regular Qt widget for rendering "
                 "which "
    ...
    this->meshingChordalDeflection.setDescription(
        textIdTr("For the tessellation of faces the chordal deflection limits "
                 "the distance between "
                 "a curve and its tessellation"));
    this->meshingAngularDeflection.setDescription(
        textIdTr("For the tessellation of faces the angular deflection limits "
                 "the angle between "
                 "subsequent segments in a polyline"));
    this->meshingRelative.setDescription(
        textIdTr("Relative computation of edge tolerance\n\n"
                 "If activated, deflection used for the polygonalisation of each "
                 "edge will be "
                 "`ChordalDeflection` &#215; `SizeOfEdge`. The deflection used "
                 "for the faces will be "
                 "the maximum deflection of their edges."));

    // Graphics
    this->navigationStyle.setDescription(
        textIdTr("3D view manipulation shortcuts configuration to mimic other "
                 "common CAD applications"));
    this->turnViewAngleIncrement.setDescription(
        textIdTr("Angle increment used to turn(rotate) the 3D view around the "
                 "normal of the view plane(Z axis frame reference)"));
...
{
    if (prop == &this->meshDefaultsColor || prop == &this->meshDefaultsEdgeColor ||
        prop == &this->meshDefaultsMaterial || prop == &this->meshDefaultsShowEdges ||
