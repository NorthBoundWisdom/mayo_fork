...
public:
    ...
    {
    public:
        ProxyItemDelegate(const ItemViewButtons *itemBtns, QStyledItemDelegate *srcDelegate,
...
void ItemViewButtons::Private::paintButton(ButtonInfo *btnInfo, QPainter *painter,
                                           const QStyleOptionViewItem &option)
{
    if (!btnInfo || !painter)
        return;
 ...
 *  \brief This signal is emitted when a button previously added with
 * addButton() is clicked (i.e. pressed down then released while the mouse
    ...
    if (object == this->itemView()->viewport())
    {
        ...
        case QEvent::ToolTip:
        {
            const QString toolTip =
                d->m_buttonUnderMouse ? d->m_buttonUnderMouse->toolTip : QString();
            if (!toolTip.isEmpty())
            {
...
void ItemViewButtons::paint(QPainter *painter, const QStyleOptionViewItem &option,
                            const QModelIndex &index) const
{
...
void ItemViewButtons::addButton(int btnId, const QIcon &icon, const QString &toolTip)
{
    ...
    {
        qWarning()
            << QString("%1 : there is already a button of index '%2'").arg(Q_FUNC_INFO).arg(btnId);
    }
}
 ...
 */
QStyledItemDelegate *ItemViewButtons::createProxyItemDelegate(QStyledItemDelegate *sourceDelegate,
