...
private:
    ...
    // Parameter 'strFilepath' is the filepath to the texture as specified by the
    // assimp material Parameter 'modelFilepath' is the filepath to the 3D model
    // being imported with Reader::readFile()
    OccHandle<Image_Texture> findOccTexture(const std::string &strFilepath,
                                            const FilePath &modelFilepath);

    // Create XCAFDoc_VisMaterial from assimp material
    // Parameter 'modelFilepath' is the filepath to the 3D model being imported
    // with Reader::readFile()
    OccHandle<XCAFDoc_VisMaterial> createOccVisMaterial(const aiMaterial *material,
                                                        const FilePath &modelFilepath);

    void transferSceneNode(const aiNode *node, DocumentPtr targetDoc, const TDF_Label &labelEntity,
