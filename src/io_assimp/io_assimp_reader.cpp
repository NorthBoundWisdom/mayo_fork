...
bool deep_aiNodeTransformationHasScaling(const aiNode *node)
{
    ...
    deep_aiNodeVisit(node,
                     [&](const aiNode *node)
                     {
                         if (!hasScaling)
                         {
                             const aiVector3D scaling = aiMatrixScaling(node->mTransformation);
                             hasScaling = !MathUtils::fuzzyEqual(scaling.x, 1) ||
                                          !MathUtils::fuzzyEqual(scaling.y, 1) ||
                                          !MathUtils::fuzzyEqual(scaling.z, 1) || scaling.x < 0. ||
                                          scaling.y < 0. || scaling.z < 0.;
                         }
                     });
...
OccHandle<Image_Texture> createOccTexture(const aiTexture *texture)
{
    ...
    auto buff = makeOccHandle<NCollection_Buffer>(NCollection_BaseAllocator::CommonBaseAllocator(),
                                                  textureSize);
...
OccHandle<Poly_Triangulation> createOccTriangulation(const aiMesh *mesh)
{
    ...
    const bool hasUvNodes =
        mesh->HasTextureCoords(textureIndex) && mesh->mNumUVComponents[textureIndex] == 2;
    auto triangulation =
        makeOccHandle<Poly_Triangulation>(mesh->mNumVertices, mesh->mNumFaces, hasUvNodes);
    ...
    for (unsigned i = 0; i < mesh->mNumFaces; ++i)
    {
        ...
        MeshUtils::setTriangle(triangulation, i + 1,
                               Poly_Triangle(indices[0] + 1, indices[1] + 1, indices[2] + 1));
    }
...
bool AssimpReader::readFile(const FilePath &filepath, TaskProgress *progress)
{
    ...
    const unsigned flags = aiProcess_Triangulate | aiProcess_JoinIdenticalVertices
        ...
        ;
    // const unsigned flags = aiProcess_Triangulate | aiProcess_FlipUVs |
    // aiProcess_CalcTangentSpace;
    ...
    for (unsigned i = 0; i < m_scene->mNumMeshes; ++i)
    {
        ...
        else if (mesh->mPrimitiveTypes & aiPrimitiveType_LINE)
        {
            ...
            this->messenger()->emitWarning(
                AssimpReaderI18N::textIdTr("LINE primitives not supported yet"));
        }
        else
        {
            this->messenger()->emitWarning(
                AssimpReaderI18N::textIdTr("Some primitive not supported"));
        }
    }
...
TDF_LabelSequence AssimpReader::transfer(DocumentPtr doc, TaskProgress *progress)
{
    ...
    deep_aiNodeVisit(m_scene->mRootNode,
                     [=](const aiNode *node)
                     {
                         aiNodeData nodeData;
                         ...
                         if (itParentData != m_mapNodeData.cend())
                         {
                             ...
                             nodeData.aiAbsoluteTrsf =
                                 parentData.aiAbsoluteTrsf * node->mTransformation;
                         }
                         else
                         {
    ...
    const TDF_Label labelEntity =
        doc->xcaf().shapeTool()->AddShape(shapeEntity, true /*makeAssembly*/);
...
OccHandle<Image_Texture> AssimpReader::findOccTexture(const std::string &strFilepath,
                                                      const FilePath &modelFilepath)
{
    // Texture might be embedded
    {
        // Note: aiScene::GetEmbeddedTextureAndIndex() isn't available for version
        // < 5.1
        const aiTexture *texture = m_scene->GetEmbeddedTexture(strFilepath.c_str());
        OccHandle<Image_Texture> occTexture = CppUtils::findValue(texture, m_mapEmbeddedTexture);
        if (occTexture)
            return occTexture;
    }
    ...
    {
        auto texture =
            makeOccHandle<Image_Texture>(filepathTo<TCollection_AsciiString>(*ptrTextureFilepath));
        // Cache texture
...
OccHandle<XCAFDoc_VisMaterial> AssimpReader::createOccVisMaterial(const aiMaterial *material,
                                                                  const FilePath &modelFilepath)
{
    ...
    std::transform(suffix.cbegin(), suffix.cend(), suffix.begin(),
    ...
    auto fnGetColor4D = [=](const char *key, unsigned type, unsigned index, aiColor4D *ptrColor,
                            bool *ptrDefinedFlag = nullptr)
    {
        const aiReturn res = material->Get(key, type, index, *ptrColor);
        // Some models have wrong color components outside [0, 1] range, and
        // Quantity_Color reject them(exception in constructor)
    ...
    auto fnGetReal = [=](const char *key, unsigned type, unsigned index, ai_real *ptrValue,
                         bool *ptrDefinedFlag = nullptr)
    {
    ...
    auto fnGetTexture =
        [=](aiTextureType type, aiString *ptrTexture, bool *ptrDefinedFlag = nullptr)
    {
...
#if 0 // Emmissive color disabled as it's making models in Mayo to be rendered
      // nearly with white color
...

    {
        ...
        if (fnGetReal(AI_MATKEY_SHININESS, &value, &matCommon.IsDefined))
        {
            ...
            // this->messenger()->trace() << "Shininess: " << value << " max: " <<
            // shininessMax;
        }
    }
...

    {
        ...
        if (fnGetReal(AI_MATKEY_REFRACTI, &value, &matPbr.IsDefined))
        {
            ...
            // If < 1 then an exception is thrown by
...
void AssimpReader::transferSceneNode(const aiNode *node, DocumentPtr targetDoc,
                                     const TDF_Label &labelEntity,
                                     const std::function<void(const aiMesh *)> &fnCallbackMesh)
{
    ...
    const aiNodeData nodeData = CppUtils::findValue(node, m_mapNodeData);
    ...
    for (unsigned imesh = 0; imesh < node->mNumMeshes; ++imesh)
    {
        ...
        if (hasScaleFactor(nodeScale))
        {
            ...
            for (int i = 1; i <= triangulation->NbNodes(); ++i)
            {
                ...
                MeshUtils::setNode(
                    triangulation, i,
                    gp_Pnt{pnt.X() * nodeScale.x, pnt.Y() * nodeScale.y, pnt.Z() * nodeScale.z});
            }
        }
#endif
        ...
        const TDF_Label labelComponent =
            targetDoc->xcaf().shapeTool()->AddComponent(labelEntity, face);
        ...
        if (mesh->mMaterialIndex < m_vecMaterial.size())
        {
            ...
            const TDF_Label materialLabel = CppUtils::findValue(material, m_mapMaterialLabel);
            if (!materialLabel.IsNull())
                targetDoc->xcaf().visMaterialTool()->SetShapeMaterial(labelFace, materialLabel);
            else
                this->messenger()->trace()
                    << "Material not found(umap), index: " << mesh->mMaterialIndex;
        }
