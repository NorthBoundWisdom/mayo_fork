...
private:
    ...
    static void amf_getDocumentElement(const void *cookie, enum gmio_amf_document_element element,
                                       uint32_t elementIndex, void *ptrElement);

    static void amf_getDocumentElementMetadata(const void *cookie,
                                               enum gmio_amf_document_element element,
                                               uint32_t elementIndex, uint32_t metadataIndex,
                                               struct gmio_amf_metadata *ptrMetadata);

    static void amf_getObjectMesh(const void *cookie, uint32_t objectIndex, uint32_t meshIndex,
                                  struct gmio_amf_mesh *ptrMesh);

    static void
    amf_getObjectMeshElement(const void *cookie,
                             const struct gmio_amf_object_mesh_element_index *elementIndex,
                             void *ptrElement);

    static void
    amf_getObjectMeshVolumeTriangle(const void *cookie,
                                    const struct gmio_amf_object_mesh_element_index *volumeIndex,
                                    uint32_t triangleIndex, struct gmio_amf_triangle *ptrTriangle);

    static void amf_getConstellationInstance(const void *cookie, uint32_t constellationIndex,
