        ...
        : PropertyGroup(parentGroup)
    {
        ...
        this->float64Format.setDescriptions(
            {{FloatTextFormat::Decimal, textIdTr("Decimal floating point(ex: 392.65)")},
             ...
             {FloatTextFormat::Shortest,
              textIdTr("Use the shortest representation: decimal or scientific")}});
        ...
        this->useZip64.setDescription(fmt::format(textIdTr("Use the ZIP64 format extensions.\n"
                                                           "Only applicable if option `{}` is on"),
                                                  this->createZipArchive.label()));
    }
...
bool GmioAmfWriter::transfer(Span<const ApplicationItem> spanAppItem, TaskProgress *progress)
{
            ...
            do
            {
                ...
                if (itParent != 0)
                {
                    const std::string name =
                        to_stdString(CafUtils::labelAttrStdName(modelTree.nodeData(itParent)));
                    if (!name.empty())
                        absoluteName += '/' + name;
                    else
    ...
    for (const ApplicationItem &appItem : spanAppItem)
    {
        ...
        {
            traverseTree(appItem.documentTreeNode().id(), modelTree,
                         [&](TreeNodeId id) { fnCreateObject(modelTree, id); });
        }
    }
...
bool GmioAmfWriter::writeFile(const FilePath &filepath, TaskProgress *progress)
{
    ...
    amfOptions.zip_entry_filename_len =
        CppUtils::safeStaticCast<uint16_t>(m_params.zipEntryFilename.size());
...
int GmioAmfWriter::createObject(const TDF_Label &labelShape)
{
    ...
    if (!shape.IsNull())
    {
        BRepUtils::forEachSubFace(shape,
    ...
    if (doc && doc->xcaf().hasShapeColor(labelShape))
    {
        ...
        auto itColor = std::find_if(m_vecMaterial.cbegin(), m_vecMaterial.cend(),
                                    [=](const Material &mat) { return mat.color == color; });
...
void GmioAmfWriter::amf_getDocumentElement(const void *cookie, gmio_amf_document_element element,
                                           uint32_t elementIndex, void *ptrElement)
{
...
void GmioAmfWriter::amf_getDocumentElementMetadata(const void *cookie,
                                                   gmio_amf_document_element element,
                                                   uint32_t elementIndex, uint32_t metadataIndex,
...
void GmioAmfWriter::amf_getObjectMesh(const void *cookie, uint32_t objectIndex, uint32_t meshIndex,
                                      gmio_amf_mesh *ptrMesh)
{
    auto writer = GmioAmfWriter::from(cookie);
...
void GmioAmfWriter::amf_getObjectMeshElement(const void *cookie,
                                             const gmio_amf_object_mesh_element_index *elementIndex,
                                             void *ptrElement)
{
    auto writer = GmioAmfWriter::from(cookie);
...
void GmioAmfWriter::amf_getObjectMeshVolumeTriangle(
    const void *cookie, const gmio_amf_object_mesh_element_index *volumeIndex,
    uint32_t triangleIndex, gmio_amf_triangle *ptrTriangle)
{
    auto writer = GmioAmfWriter::from(cookie);
...
void GmioAmfWriter::amf_getConstellationInstance(const void *cookie, uint32_t constellationIndex,
