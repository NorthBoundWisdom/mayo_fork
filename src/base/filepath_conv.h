...
template <typename T>
T filepathTo(const FilePath &fp)
{
...
template <>
struct FilePathConv<TCollection_AsciiString>
{
    static auto to(const FilePath &fp)
    {
        return TCollection_AsciiString(fp.string().c_str());
    }
};

// FilePath -> TCollection_ExtendedString
template <>
struct FilePathConv<TCollection_ExtendedString>
{
    static auto to(const FilePath &fp)
    {
        // Can't use "if constexpr(std::is_same_v<FilePath::value_type::char>)" here
        // because discarded statements are compiled. On Windows compilation would
        // fail.
