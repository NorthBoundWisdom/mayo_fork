...
struct TaskManager::Private
{
    ...
    // Const/mutable functions to find an Entity from a task identifier. Returns
    // null if not found
...
TaskManager::~TaskManager()
{
    ...
    // Erase the task from its container before destruction, this will allow
    // TaskProgress destructor to behave correctly(it calls
    // TaskProgress::setValue())
