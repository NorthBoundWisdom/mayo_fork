...
public:
    ...
    static TriangulationAnnexDataPtr Set(const TDF_Label &label,
                                         std::vector<Quantity_Color> &&vecNodeColor);

    Span<const Quantity_Color> nodeColors() const
    {
        return m_vecNodeColor;
    }
    ...
    void Paste(const OccHandle<TDF_Attribute> &into,
               const OccHandle<TDF_RelocationTable> &table) const override;
