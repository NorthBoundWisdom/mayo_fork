...
Enumeration &Enumeration::chopPrefix(std::string_view prefix)
{
    ...
    {
        if (item.name.key.substr(0, prefix.size()) ==
            prefix) // TODO Replace with C++20 starts_with()
...
{
    auto it = std::find_if(m_vecItem.cbegin(), m_vecItem.cend(),
...
{
    auto itFound =
        std::find_if(m_vecItem.cbegin(), m_vecItem.cend(),
