...
public:
    ...
    {
        // findShapeLabel() will first try to find the shape among the top-level
        // shapes
        FindShapeLabel_Instance = 0x01,
        // findShapeLabel() will try to find the shape find the shape among the
        // components of assemblies
        FindShapeLabel_Component = 0x02,
        // findShapeLabel() will try tries to find a shape as a subshape of
        // top-level simple shapes
    ...
    TDF_Label findShapeLabel(const TopoDS_Shape &shape,
                             FindShapeLabelFlags flags = FindShapeLabel_All) const;

    TopLoc_Location shapeAbsoluteLocation(TreeNodeId nodeId) const;
    static TopLoc_Location shapeAbsoluteLocation(const Tree<TDF_Label> &modelTree,
                                                 TreeNodeId nodeId);
    static TopLoc_Location shapeReferenceLocation(const TDF_Label &lbl);
    static TDF_Label shapeReferred(const TDF_Label &lbl);

    // Returns labels of the top-level free shapes that were not found in
    // 'seqOther'
...
private:
    ...
    void setLabelMain(const TDF_Label &labelMain)
    {
        m_labelMain = labelMain;
    }
    void setModelTree(Tree<TDF_Label> &modelTree)
    {
