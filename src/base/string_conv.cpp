...
bool toUtf16String(std::string_view str, UINT localeAcp, std::vector<wchar_t> &utf16)
{
    ...
    const int lenUtf16 =
        MultiByteToWideChar(localeAcp, MB_ERR_INVALID_CHARS, str.data(), lenStr, nullptr, 0);
    if (lenUtf16 == 0)
        return {};

    // Encode to utf16 string
    utf16.resize(lenUtf16 + 1);
    const int convCount = MultiByteToWideChar(localeAcp, MB_ERR_INVALID_CHARS, str.data(), lenStr,
                                              utf16.data(), lenUtf16);
    if (convCount == 0)
        utf16.resize(1);
...
std::string toUtf8String(std::string_view str, const std::locale &locale)
{
    ...
    const int lenUtf16 = CppUtils::safeStaticCast<int>(utf16.size()) - 1;
    const int lenUtf8 =
        WideCharToMultiByte(CP_UTF8, 0, utf16.data(), lenUtf16, nullptr, 0, nullptr, nullptr);
...
std::string to_stdString(double value, const DoubleToStringOptions &opts)
{
    ...
    // Helper function to erase the last character of the string pointed to by
    // 'str'
    ...
    if (opts.removeTrailingZeroes)
    {
        ...
        if (str.find(chDecPnt) != std::string::npos)
