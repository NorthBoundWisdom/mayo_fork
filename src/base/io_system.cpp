...
void dispatchErrors(std::string_view headerMsg, const MessageCollecter &msgCollect,
                    Messenger *target)
{
...
void dispatchWarnings(std::string_view headerMsg, const MessageCollecter &msgCollect,
                      Messenger *target)
{
...
bool System::importInDocument(const Args_ImportInDocument &args)
{
    ...
    auto fnPostProcess = [&](TaskData &taskData)
    {
        ...
        TaskProgress progress(taskData.progress, args.entityPostProcessProgressSize,
    ...
    {
        // Need to call Document::addEntityTreeNodeSequence() instead of
        // addEntityTreeNode() in for() loop. The former function doesn't interleave
        // update of the model tree and emission of "entity added" signal for each
        // entity. This prevents data race to happen on the Document's model tree
        // within slots connected to signal(and living in other threads)
    ...
    { // Many files case
        ...
        childTaskManager.signalProgressChanged.connectSlot(
            [&](TaskId, int) { rootProgress->setValue(childTaskManager.globalProgress()); });

        // Read files
        for (TaskData &taskData : vecTaskData)
        {
            taskData.filepath = listFilepath[&taskData - &vecTaskData.front()];
            taskData.taskId = childTaskManager.newTask(
                [&](TaskProgress *progressChild)
                {
        ...
        {
            auto it = std::find_if(vecTaskData.begin(), vecTaskData.end(),
                                   [&](const TaskData &taskData)
                                   {
                                       return !taskData.transferred &&
                                              childTaskManager.waitForDone(taskData.taskId, 25);
                                   });
...
void System::visitUniqueItems(Span<const ApplicationItem> spanItem,
...
void System::traverseUniqueItems(Span<const ApplicationItem> spanItem,
                                 std::function<void(const DocumentTreeNode &)> fnCallback,
                                 TreeTraversal mode)
{
    System::visitUniqueItems(
        spanItem,
        [=](const ApplicationItem &item)
        {
            ...
            else if (item.isDocumentTreeNode())
            {
                ...
                traverseTree(
                    docTreeNodeId, modelTree, [&](TreeNodeId id) { fnCallback({doc, id}); }, mode);
            }
        });
}
...
System::Operation_ImportInDocument::Operation &
System::Operation_ImportInDocument::withEntityPostProcess(
    std::function<void(TDF_Label, TaskProgress *)> fn)
{
...
System::Operation_ImportInDocument::Operation &
System::Operation_ImportInDocument::withEntityPostProcessInfoProgress(int progressSize,
                                                                      std::string_view progressStep)
{
...
Format probeFormat_STL(const System::FormatProbeInput &input)
{
    ...
    // Binary STL ?
    {
        ...
        if (sample.size() >= binaryStlHeaderSize)
        {
            ...
            const uint32_t facetsCount = bytes[offset] | (bytes[offset + 1] << 8) |
