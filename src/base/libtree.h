...
//     * accessible through an identifier which is unique in the tree it belongs
//     to
//     * null identifier(0) refers to null(void) tree node
//     * single parent node. A tree node is a root in case its parent identifier
//     is null
//     * has 0..N child tree nodes. A tree node is a leaf in case it has no
//     child
//     * owns data of any type, though data type is the same for all nodes of
//     the same tree
//
// Storage of nodes and associated data is memory efficient : all nodes are
// stored in a single array Use the traverseTree_() family of functions to visit
// nodes of a Tree object
//
...
public:
    ...
    // Identifier of the parent of node 'id'. Might returns 0(in such case 'id' is
    // a root)
    ...
    // Data associated to node of identifier 'id' or default-constructed object of
    // type 'T' in case of error
    const T &nodeData(TreeNodeId id) const;

    // Is node of identifier 'id' a root? Note: a root as parent nodes(ie
    // nodeParent(id) == 0)
    ...
    // Appends child to node identified by 'parentId'. That new node will contain
    // 'data'
    ...
    // Total count of nodes in the tree, this includes also nodes marked as
    // "deleted" but not yet destroyed from the tree
...
template <typename T>
TreeNodeId Tree<T>::nodeSiblingPrevious(TreeNodeId id) const
{
...
template <typename T>
TreeNodeId Tree<T>::nodeParent(TreeNodeId id) const
{
    const TreeNode *node = this->ptrNode(id);
...
template <typename T>
bool Tree<T>::nodeIsRoot(TreeNodeId id) const
{
    const TreeNode *node = this->ptrNode(id);
...
template <typename T>
bool Tree<T>::nodeIsLeaf(TreeNodeId id) const
{
...
template <typename T>
bool Tree<T>::isNodeDeleted(TreeNodeId id) const
{
...
void Tree<T>::removeRoot(TreeNodeId id)
{
    ...
    traverseTree_postOrder(id, *this,
                           [=](TreeNodeId visitId)
                           {
