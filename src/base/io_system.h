...
public:
    ...
    Span<const Format> readerFormats() const
    {
        return m_vecReaderFormat;
    }
    Span<const Format> writerFormats() const
    {
        return m_vecWriterFormat;
    }
    ...
    struct Args_ImportInDocument
    {
        ...
        // Optional: provider of format-specific parameters to be considered when
        // reading
        const ParametersProvider *parametersProvider = nullptr;

        // Optional: function applied to each imported entity. Executed before
        // adding entities into target document
        ...
        // Optional: predicate telling whether imported entities have to be
        // post-processed(ie whether
        //           `entityPostProcess` function has to be called)
        // The single argument being the format of the file from which entities were
        // read
        ...
        // Optional: the messenger object used to report any additional infos,
        // warnings and errors
        Messenger *messenger = nullptr;

        // Optional: the indicator object used to report progress of the import
        // operation
    ...
    struct Operation_ImportInDocument
    {
        ...
        Operation &withEntityPostProcessInfoProgress(int progressSize,
    ...
    // Helper struct to provide fluent-like interface over
    // Args_ExportApplicationItems See also
    ...
    // Iterate over `spanItem` and call `fnCallback` for each item. Guarantees
    // that doublon items will be visited only once
    static void visitUniqueItems(Span<const ApplicationItem> spanItem,
                                 std::function<void(const ApplicationItem &)> fnCallback);

    // Iterate over `spanItem` and then deep traverse the corresponding tree node
    // to call `fnCallback` for each item. Guarantees that doublon items will be
    // visited only once
    static void traverseUniqueItems(Span<const ApplicationItem> spanItem,
