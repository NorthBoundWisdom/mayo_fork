...
public:
    ...
    static OccHandle<TransientType> makeHandle(const TransientType *ptr)
    {
        return ptr;
    }
...
#endif
    static ReturnType_StartProgressIndicator
    start(const OccHandle<Message_ProgressIndicator> &progress);

    // Encodes 'color' into hexadecimal representation with #RRGGBB format
    static std::string colorToHex(const Quantity_Color &color);

    // Decodes a string containing a color with #RRGGBB format to a Quantity_Color
    // object RR, GG, BB are in hexadecimal notation
    static bool colorFromHex(std::string_view strHex, Quantity_Color *color);

    // Returns the type to be used(by default) for RGB colors, depending on
    // OpenCascasde version
    static Quantity_TypeOfColor preferredRgbColorType();

    // Returns a linear-space RGB color from input 'color' expressed with
    // preferredRgbColorType()
