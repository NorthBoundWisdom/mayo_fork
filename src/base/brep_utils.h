...
struct BRepUtils
{
    ...
    // Complexity here is the degree of abstraction provided(eg face type is more
    // complex than edge type)
    static bool moreComplex(TopAbs_ShapeEnum lhs, TopAbs_ShapeEnum rhs);

    // Returns hash code computed from 'shape'
    // Computation uses the internal TShape and Location, but Orientation is not
    // considered Returned hash code is in the range [1, max(int)]
    ...
    // Deserializes string 'str' obtained from 'shapeToToString()' into a shape
    // object
    ...
    // Computes a mesh representation of 'shape' using OpenCascade meshing
    // algorithm
