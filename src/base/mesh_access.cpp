...
public:
    XCafFace_MeshAccess(const DocumentTreeNode &treeNode, const TopoDS_Face &face)
    {
        ...
        const TopLoc_Location locShape =
            XCaf::shapeAbsoluteLocation(doc->modelTree(), treeNode.id());
...
private:
    static std::optional<Quantity_Color> findShapeColor(const DocumentPtr &doc,
                                                        const TDF_Label &labelShape)
    {
