...
static gp_XYZ xyzFromString(std::string_view str)
{
    ...
    auto fnParseNextCoord = [&]
    {
        ...
        ptrCoord =
            std::find_if(ptrCoord, ptrCoordEnd, [&](char ch) { return !std::isspace(ch, locale); });
...
PropertyValueConversion::Variant PropertyValueConversion::toVariant(const Property &prop) const
{
    auto fnError = [&](std::string_view text)
    {
        ...
        std::cerr << fmt::format("PropertyValueConversion::toVariant() {} [ "
    ...
    else if (isType<PropertyOccPnt>(prop))
    {
        try
        {
            return toString(constRef<PropertyOccPnt>(prop).value().XYZ(),
    ...
    else if (isType<PropertyOccVec>(prop))
    {
        try
        {
            return toString(constRef<PropertyOccVec>(prop).value().XYZ(),
                            m_doubleToStringPrecision);
        }
        catch (const std::exception &err)
        {
    ...
    else if (isType<BasePropertyQuantity>(prop))
    {
        ...
        const auto trRes =
            UnitSystem::translate(UnitSystem::SI, qtyProp.quantityValue(), qtyProp.quantityUnit());
        try
        {
...
bool PropertyValueConversion::fromVariant(Property *prop, const Variant &variant) const
{
    ...
    {
        // TODO Use other output stream(dedicated Messenger object?)
        std::cerr << fmt::format("PropertyValueConversion::fromVariant() {} [ "
                                 "propertyName={}, propertyType={} ]",
                                 text, prop->name().key, prop->dynTypeName())
                  << std::endl;
        return false;
    ...
    else if (isType<PropertyFilePath>(prop))
    {
        ...
        //       If character type of the source string is "char" then FilePath
        //       constructor assumes native narrow encoding(which might cause
        //       encoding issues)
        if (variant.isConvertibleToConstRefString())
    ...
    else if (isType<PropertyOccPnt>(prop))
    {
        if (variant.isConvertibleToConstRefString())
        {
            try
            {
                return ptr<PropertyOccPnt>(prop)->setValue(
                    xyzFromString(variant.toConstRefString()));
            }
            catch (const std::exception &err)
            {
                return fnError(fmt::format("Failed with error '{}'", err.what()));
    ...
    else if (isType<PropertyOccVec>(prop))
    {
        if (variant.isConvertibleToConstRefString())
        {
            try
            {
                return ptr<PropertyOccVec>(prop)->setValue(
                    xyzFromString(variant.toConstRefString()));
            }
            catch (const std::exception &err)
            {
    ...
    else if (isType<PropertyEnumeration>(prop))
    {
        if (variant.isConvertibleToConstRefString())
        {
            ...
            const Enumeration::Item *ptrItem =
                ptr<PropertyEnumeration>(prop)->enumeration().findItemByName(name);
            if (ptrItem)
