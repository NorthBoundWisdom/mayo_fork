...
public:
    ...
    void loadPropertyFrom(const Settings::Storage &source, std::string_view sectionPath,
                          Property *property)
    {
        if (!property)
            return;
        ...
        if (source.contains(settingPath))
        {
            ...
            if (!ok)
            {
                ...
                std::cerr << fmt::format("Failed to load setting [path={}]", settingPath)
                          << std::endl;
            }
        }
    }
            ...
            for (const Settings_Setting &setting : section.vecSetting)
            {
                ...
                if (!fnExclude || !fnExclude(*prop))
                {
                    ...
                    const std::string settingPath =
                        std::string(sectionPath).append("/").append(propKey);
