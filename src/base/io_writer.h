...
public:
    ...
    // Apply properties contain in 'group' to the writer's parameter values(known
    // in writer sub-class)
...
public:
    ...
    // Returns supported formats, ie the formats this factory can create writers
    // for
    virtual Span<const Format> formats() const = 0;

    // Creates and returns a Writer object that matches the given format, or
    // nullptr if no matching writer is found
    ...
    // properties is a generic way to change parameter values of a Writer object
    // corresponding to format(see also Writer::applyProperties())
