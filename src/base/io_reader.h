...
public:
    ...
    // Converts data read during readFile() step into document 'doc' using
    // indicator to report progress Returns the list of entities added to document
    // 'doc'
    virtual TDF_LabelSequence transfer(DocumentPtr doc, TaskProgress *progress) = 0;

    // Apply properties contain in 'group' to the reader's parameter values(known
    // in reader sub-class)
...
public:
    ...
    // Returns supported formats, ie the formats this factory can create readers
    // for
    virtual Span<const Format> formats() const = 0;

    // Creates and returns a Reader object that matches the given format, or
    // nullptr if no matching reader is found
    virtual std::unique_ptr<Reader> create(Format format) const = 0;

    // Creates and returns properties that match the given format. Those
    // properties is a generic way to change parameter values of a Reader object
    // corresponding to format(see also Reader::applyProperties())
    virtual std::unique_ptr<PropertyGroup> createProperties(Format format,
