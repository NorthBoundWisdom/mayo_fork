...
public:
    ...
    // Iterates over name of items and removes 'prefix' string that may appear at
    // the beginning
    ...
    // Finds index of the item corresponding to an enumerated value. Returns -1 if
    // not found
    template <typename EnumType>
    int findIndexByValue(EnumType value) const;

    // Finds the item corresponding to an enumerated value. Returns nullptr if not
    // found
    template <typename EnumType>
    const Item *findItemByValue(EnumType value) const;
    ...
    template <typename EnumType>
    std::string_view findNameByValue(EnumType value) const;

    // Finds the enumerated value corresponding to a name. Throws exception if not
    // found
    ...
    // Creates an Enumeration object from an enumerated type, using MetaEnum
    // helper Note: client code has to include header "enumeration_fromenum.h"
    template <typename EnumType>
...
template <typename ValueType>
Enumeration &Enumeration::addItem(ValueType value, const TextId &name)
{
...
{
    static_assert(std::is_enum_v<EnumType> || std::is_integral_v<EnumType>,
                  "ENUM must be an enumeration or integer type");
...
{
    static_assert(std::is_enum_v<EnumType> || std::is_integral_v<EnumType>,
                  "EnumType must be an enumeration or integer type");
...
template <typename EnumType>
std::string_view Enumeration::findNameByValue(EnumType value) const
{
