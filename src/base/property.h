...
// PropertyGroup defines callbacks to be executed when special events happen on
// contained properties:
//     - a property value is about to be changed
//     - a property value was changed
//     - the "enabled" status of a property was toggled
// A PropertyGroup can be linked to a parent group(optional). In such case the
// child group executes as well the parent group's callbacks.
class PropertyGroup
{
public:
...
// It blocks call to PropertyGroup::onPropertyChanged() in its constructor and
...
// Provides an abstract storage of a value with associated meta-data(name,
// description, ...)
class Property
{
public:
    ...
    void setDescription(std::string_view text)
    {
        m_description = text;
    }
...
template <typename T>
bool Property::setValueHelper(Property *prop, T *ptrValue, const T &newValue)
{
    bool okResult = true;
    if (prop->hasGroup())
    {
