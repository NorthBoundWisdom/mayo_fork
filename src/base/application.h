...
public: // -- from TDocStd_Application
    ...
    void NewDocument(const TCollection_ExtendedString &format,
                     OccHandle<CDM_Document> &outDoc) override;
    void InitDocument(const OccHandle<CDM_Document> &doc) const override;
#else
    void NewDocument(const TCollection_ExtendedString &format,
                     OccHandle<TDocStd_Document> &outDoc) override;
    void InitDocument(const OccHandle<TDocStd_Document> &doc) const override;
#endif

    // TODO: Redefine TDocStd_Document::BeforeClose() to emit signal
    // documentClosed class Document : public TDocStd_Document { ... }; using
    // DocumentPtr = OccHandle<Document>
    // -> Can't do because PCDM_RetrievalDriver subclasses create explicitly "new
    // TDocStd_Document(...)"
