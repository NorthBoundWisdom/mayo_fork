...
{
    return this->isValidTextIndex(i) ? m_textProps.at(i).m_aspect->Aspect() :
                                       OccHandle<Graphic3d_AspectText3d>();
}
...
void AIS_Text::Compute(const OccHandle<PrsMgr_PresentationManager> &,
                       const OccHandle<Prs3d_Presentation> &pres, const int)
{
