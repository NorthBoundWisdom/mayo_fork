...
public:
    ...
    {
        Empty,   // Omit name
        Product, // Product name(eg from XCaf::shapeReferred(), which could be
                 // shared by multiple instances)
        Instance,
        InstanceOrProduct,  // Instance name when available and Product name
                            // otherwise
        ProductOrInstance,  // Product name when available and Instance name
                            // otherwise
