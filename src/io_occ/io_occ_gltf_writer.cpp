...
public:
        ...
        : PropertyGroup(parentGroup)
    {
        ...
        this->transformationFormat.setDescriptions(
            {{RWGltf_WriterTrsfFormat_Compact, textIdTr("Automatically choose most compact "
             ...
             {RWGltf_WriterTrsfFormat_TRS,
              textIdTr("Transformation decomposed into Translation "
                       "vector, Rotation quaternion and Scale factor(T * R * S)")}});

        this->format.mutableEnumeration().changeTrContext(
            OccGltfWriter::Properties::textIdContext());
        ...
        this->embedTextures.setDescription(
            fmt::format(textIdTr("Write image textures into target file.\n\n"
                                 "If set to `false` then texture images will be written as "
                                 "separate files.\n\n"
                                 "Applicable only if option `{0}` is set to `{1}`"),
                        this->format.label(), MetaEnum::name<Format>(Format::Binary)));
        ...
        this->keepIndices16b.setDescription(
            fmt::format(textIdTr("Prefer keeping 16-bit indexes while merging face.\n\n"
                                 "May reduce binary data size thanks to smaller triangle "
                                 "indexes.\n\n"
    ...
    PropertyEnum<RWMesh_CoordinateSystem> inputCoordinateSystem{this,
                                                                textId("inputCoordinateSystem")};
    PropertyEnum<RWMesh_CoordinateSystem> outputCoordinateSystem{this,
                                                                 textId("outputCoordinateSystem")};
    PropertyEnum<RWGltf_WriterTrsfFormat> transformationFormat{this,
                                                               textId("transformationFormat")};
...
bool OccGltfWriter::transfer(Span<const ApplicationItem> spanAppItem, TaskProgress *)
{
    ...
    System::visitUniqueItems(spanAppItem,
                             [=](const ApplicationItem &appItem)
                             {
...
bool OccGltfWriter::writeFile(const FilePath &filepath, TaskProgress *progress)
{
    ...
    writer.ChangeCoordinateSystemConverter().SetInputCoordinateSystem(
        m_params.inputCoordinateSystem);
    writer.ChangeCoordinateSystemConverter().SetOutputCoordinateSystem(
        m_params.outputCoordinateSystem);
#if OCC_VERSION_HEX >= 0x070600
    ...
    {
        return fmt::format(Properties::textIdTr("Option supported from OpenCascade ≥ v7.6 "
