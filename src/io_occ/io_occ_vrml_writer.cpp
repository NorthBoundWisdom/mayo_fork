...
public:
    ...
    PropertyEnum<VrmlAPI_RepresentationOfShape> shapeRepresentation{this,
                                                                    textId("shapeRepresentation")};
};
...
bool OccVrmlWriter::transfer(Span<const ApplicationItem> spanAppItem, TaskProgress *progress)
{
...
#else
            // TODO Call VrmlData_ShapeConvert::AddShape() on each child entity of
            // "shape" type
#endif
                                 }
    ...
    converter.Convert(rep == VrmlAPI_ShadedRepresentation || rep == VrmlAPI_BothRepresentation,
...
void OccVrmlWriter::applyProperties(const PropertyGroup *params)
{
    ...
    if (ptr)
    {
        m_params.shapeRepresentation =
            static_cast<VrmlAPI_RepresentationOfShape>(ptr->shapeRepresentation);
    }
}
