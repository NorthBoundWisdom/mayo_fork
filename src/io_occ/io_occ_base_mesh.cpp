    ...
    , systemLengthUnit(this, textId("systemLengthUnit"))
{
    ...
    this->systemLengthUnit.setDescription(
        textIdTr("System length units to convert into while reading files"));
}
...
// Helper function to fix potential OpenCacsade exception throwing with
// unsupported scale transformations
//     To be valid(for OpenCascade), a BRep shape transform should only be a
//     composition of translations and rotations. Scale factors <> 1 are not
//     supported This function traverses the assembly graph from `label` and
//     fixes any instance node placement regarding scale factor(it's forced to
//     1)
    ...
    else if (XCaf::isShapeReference(label))
    {
        ...
        if (GeomUtils::hasScaling(loc.Transformation()))
        {
            ...
            messenger->warning() << fmt::format(
                "Item \"{}\" is located with scaling transformation which is not "
                "supported(value={})",
                to_stdString(CafUtils::labelAttrStdName(label)), trsfBadScaleFactor);
        }
...
{
    m_reader.SetRootPrefix(
        string_conv<TCollection_AsciiString>(this->constParameters().rootPrefix));
    m_reader.SetSystemLengthUnit(
        OccBaseMeshReaderProperties::lengthUnitFactor(this->constParameters().systemLengthUnit));
