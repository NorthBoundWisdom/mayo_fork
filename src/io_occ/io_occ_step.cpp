...
public:
        ...
        : PropertyGroup(parentGroup)
    {
        this->productContext.setDescription(
            textIdTr("When reading AP 209 STEP files, allows selecting either only `design` "
                     ...
                     "`design`, so if this mode is set to `analysis`, nothing will be "
                     "read"));

        this->assemblyLevel.setDescription(
            textIdTr("Specifies which data should be read for the products found "
                     "in the STEP file"));

        this->preferredShapeRepresentation.setDescription(
            textIdTr("Specifies preferred type of representation of the shape of "
                     "the product, in "
                     "case if a STEP file contains more than one representation "
                     "(i.e. multiple "
                     "`PRODUCT_DEFINITION_SHAPE` entities) for a single product"));

        this->readShapeAspect.setDescription(
            textIdTr("Defines whether shapes associated with the "
                     "`PRODUCT_DEFINITION_SHAPE` entity "
                     "of the product via `SHAPE_ASPECT` should be translated.\n"
                     "This kind of association was used for the representation of "
                     "hybrid models (i.e. models "
                     "whose shape is composed of different types of "
                     "representations) in AP 203 files "
                     "before 1998, but it is also used to associate auxiliary "
                     "information with the "
                     "sub-shapes of the part. Though STEP translator tries to "
                     "recognize such cases "
                     "correctly, this parameter may be useful to avoid "
        ...
        this->productContext.setDescriptions(
            {{ProductContext::Design,
              textIdTr("Translate only products that have "
                       "`PRODUCT_DEFINITION_CONTEXT` with field `life_cycle_stage` "
                       "set to `design`")},
        ...
        this->assemblyLevel.setDescriptions(
            {{AssemblyLevel::Assembly,
              textIdTr("Translate the assembly structure and shapes "
                       "associated with parts only(not with sub-assemblies)")},
             {AssemblyLevel::Structure,
              textIdTr("Translate only the assembly structure "
                       "without shapes(a structure of empty compounds). This mode "
                       "can be useful as "
                       "an intermediate step in applications requiring specialized "
                       "processing of assembly parts")},
             {AssemblyLevel::Shape,
              textIdTr("Translate only shapes associated with the "
                       "product, ignoring the assembly structure (if any). This "
                       "can be useful to "
                       "translate only a shape associated with specific product, "
                       "as a complement to assembly mode")},
             {AssemblyLevel::All,
              textIdTr("Translate both the assembly structure and all "
                       "associated shapes. If both shape and sub-assemblies are "
                       "associated with the "
                       "same product, all of them are read and put in a single "
                       "compound")}});

        this->preferredShapeRepresentation.addDescription(
            ShapeRepresentation::All, textIdTr("Translate all representations(if "
                                               "more than one, put in compound)"));
    ...
    PropertyEnum<ShapeRepresentation> preferredShapeRepresentation{
        this, textId("preferredShapeRepresentation")};
        ...
        : PropertyGroup(parentGroup)
    {
        ...
        this->freeVertexMode.setDescriptions({
            {FreeVertexMode::Compound, textIdTr("All free vertices are united into one "
                                                "compound and exported in one shape definition "
                                                "representation (vertex name "
                                                "and style are lost)")},
            {FreeVertexMode::Single,
             textIdTr("Each vertex is exported in its own "
                      "`SHAPE DEFINITION REPRESENTATION`(vertex name and style are "
                      "not lost, but "
                      "the STEP file size increases)")},
        });

        this->writePCurves.setDescription(
            textIdTr("Indicates whether parametric curves (curves in parametric "
                     "space of surface) should be "
                     "written into the STEP file.\n"
                     "It can be disabled in order to minimize the size of the "
                     "resulting file."));
        ...
        this->headerOrganization.setDescription(
            textIdTr("Organization(of author) attribute in STEP header"));
        this->headerOriginatingSystem.setDescription(
            textIdTr("Originating system attribute in STEP header"));
...
bool OccStepWriter::transfer(Span<const ApplicationItem> appItems, TaskProgress *progress)
{
    ...
    if (m_params.schema != m_schemaLastTransfer)
    {
        ...
        // For the parameter "write.step.schema" to take effect, method
        // STEPControl_Writer::Model(true) should be called after changing this
        // parameter (corresponding command in DRAW is "newmodel")
