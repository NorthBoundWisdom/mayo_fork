...
// Provides the link between Base::Document and graphical representations(called
// "graphics objects")
class GuiDocument
{
public:
    ...
    GuiApplication *guiApplication() const
    {
        return m_guiApp;
    }
    ...
    // Gets/sets the ratio between physical pixels and device-independent pixels
    // for the target window. This value is dependent on the screen the window is
    // on, and may have to be updated when the target window is moved. Common
    // values are 1.0(default) on normal displays and 2.0 on Apple "retina"
    // displays
    ...
    // Executes callback 'fn' on all graphics objects associated to tree node
    // 'nodeId' This also includes all children(deep node traversal)
    void foreachGraphicsObject(TreeNodeId nodeId,
    ...
    // Toggles selected status of an application item(doesn't affect Application's
    // selection model)
    ...
    // -- Exploding
    double explodingFactor() const
    {
        return m_explodingFactor;
    }
    ...
    // -- Camera animation
    V3dViewCameraAnimation *viewCameraAnimation() const
    {
...
private:
    ...
    struct GraphicsEntity
    {
        struct Object
        {
            Object(const GraphicsObjectPtr &p)
                : ptr(p)
            {
            }
