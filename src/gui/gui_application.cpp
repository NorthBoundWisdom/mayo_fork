...
{
    void onApplicationItemSelectionChanged(Span<const ApplicationItem> selected,
...
{
    auto itFound =
        std::find_if(d->m_vecGuiDocument.begin(), d->m_vecGuiDocument.end(),
...
void GuiApplication::connectApplicationItemSelectionChanged(bool on)
{
    ...
    if (on)
    {
        ...
        d->m_connApplicationItemSelectionChanged =
            sigChanged.connectSlot(&Private::onApplicationItemSelectionChanged, d);
    }
}
