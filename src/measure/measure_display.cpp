...
namespace
{
struct MeasureDisplayI18N
{
    MAYO_DECLARE_TEXT_ID_FUNCTIONS(Mayo::MeasureDisplayI18N)
};
} // namespace

std::unique_ptr<IMeasureDisplay> BaseMeasureDisplay::createFrom(MeasureType type,
...
std::unique_ptr<IMeasureDisplay> BaseMeasureDisplay::createEmptySum(MeasureType type)
{
    ...
    case MeasureType::Angle: return std::make_unique<MeasureDisplayAngle>(Mayo::MeasureAngle{});
    case MeasureType::Length: return std::make_unique<MeasureDisplayLength>(Mayo::MeasureLength{});
    case MeasureType::Area: return std::make_unique<MeasureDisplayArea>(Mayo::MeasureArea{});
    default: return {};
    }
}
...
void BaseMeasureDisplay::adaptGraphics(const OccHandle<Graphic3d_GraphicDriver> &driver)
{
    ...
    for (int i = 0; i < this->graphicsObjectsCount(); ++i)
    {
        ...
        {
            // NOTE
            // Usage of Aspect_TODT_SUBTITLE is causing a crash when VBO are not
            // available(eg because of too old OpenGL version)
...
std::string BaseMeasureDisplay::text(const gp_Pnt &pnt, const MeasureDisplayConfig &config)
{
    ...
    const std::string str = fmt::format(
        ...
        text(trPntX, config), text(trPntY, config), text(trPntZ, config), trPntX.strUnit);
    return str;
}
...
void BaseMeasureDisplay::adaptScale(const OccHandle<AIS_TextLabel> &gfxText,
                                    const MeasureDisplayConfig &config)
{
    ...
    , m_gfxText(new AIS_TextLabel)
{
    ...
    BaseMeasureDisplay::applyGraphicsDefaults(this);
    // BEWARE LineAspect() is created indirectly by SetColor() call in
    // applyGraphicsDefaults() function
    if (m_gfxCircle)
...
void MeasureDisplayCircleCenter::update(const MeasureDisplayConfig &config)
{
    ...
    m_gfxText->SetText(
        to_OccExtString(BaseMeasureDisplay::graphicsText(m_circle.Location(), config)));
    BaseMeasureDisplay::adaptScale(m_gfxText, config);
}
    ...
    , m_gfxDiameterText(new AIS_TextLabel)
{
    ...
    m_gfxDiameter = new AIS_Line(new Geom_CartesianPoint(circle.pntAnchor),
                                 new Geom_CartesianPoint(otherPntAnchor));
...
void MeasureDisplayCircleDiameter::update(const MeasureDisplayConfig &config)
{
    ...
    this->setText(fmt::format(MeasureDisplayI18N::textIdTr("Diameter: {0}{1}"), strDiameter,
                              trDiameter.strUnit));

    m_gfxDiameterText->SetText(
        to_OccExtString(fmt::format(MeasureDisplayI18N::textIdTr(" Ø{0}"), strDiameter)));
...
gp_Pnt MeasureDisplayCircleDiameter::diameterOpposedPnt(const gp_Pnt &pntOnCircle,
                                                        const gp_Circ &circ)
{
...
MeasureDisplayDistance::MeasureDisplayDistance(const MeasureDistance &dist)
    : m_dist(dist)
    , m_gfxLength(
          new AIS_Line(new Geom_CartesianPoint(dist.pnt1), new Geom_CartesianPoint(dist.pnt2)))
...
void MeasureDisplayDistance::update(const MeasureDisplayConfig &config)
{
    ...
    auto fnStrDistanceType = [](MeasureDistance::Type distType) -> std::string_view
    {
        switch (distType)
        {
        case MeasureDistance::Type::Mininmum: return "Min Distance";
        case MeasureDistance::Type::CenterToCenter: return "Distance";
    ...
    if (this->sumCount() > 1)
    {
        this->setText(fmt::format(MeasureDisplayI18N::textIdTr("{0}: {1}{2}"), this->sumTextOr(""),
                                  strLength, trLength.strUnit));
    }
    else
    {
        ;
        this->setText(
            fmt::format(MeasureDisplayI18N::textIdTr("{0}: {1}{2}<br>Point1: {3}<br>Point2: {4}"),
                        fnStrDistanceType(m_dist.type), strLength, trLength.strUnit,
...
MeasureDisplayAngle::MeasureDisplayAngle(MeasureAngle angle)
    : m_angle(angle)
    , m_gfxEntity1(new AIS_Line(new Geom_CartesianPoint(angle.pntCenter),
                                new Geom_CartesianPoint(angle.pnt1)))
    , m_gfxEntity2(new AIS_Line(new Geom_CartesianPoint(angle.pntCenter),
                                new Geom_CartesianPoint(angle.pnt2)))
    , m_gfxAngleText(new AIS_TextLabel)
{
    ...
    const gp_Dir nCircle =
        fnVecIsNull(vec1) || fnVecIsNull(vec2) ? gp::DZ() : gp_Dir{vec1.Crossed(vec2)};
    ...
    m_gfxAngleText->SetPosition(
        ElCLib::Value(param1 + std::abs((param2 - param1) / 2.), geomCircle->Circ()));
}
...
void MeasureDisplayAngle::update(const MeasureDisplayConfig &config)
{
    ...
    this->setText(fmt::format(MeasureDisplayI18N::textIdTr("{0}: {1}{2}"),
                              BaseMeasureDisplay::sumTextOr(MeasureDisplayI18N::textIdTr("Angle")),
                              strAngle, trAngle.strUnit));
...
void MeasureDisplayBoundingBox::update(const MeasureDisplayConfig &config)
{
    ...
    this->setText(fmt::format(MeasureDisplayI18N::textIdTr("Min point: {0}<br>Max point: {1}<br>"
                                                           "Size: {2} x {3} x {4}{5}<br>"
                                                           "Volume: {6}{7}"),
                              ...
                              BaseMeasureDisplay::text(trZLength, config), trXLength.strUnit,
                              BaseMeasureDisplay::text(trVolume, config), trVolume.strUnit));
