...
public:
    std::string_view message() const override
    {
        switch (Err)
        {
        case ErrorCode::NotVertex: return textIdTr("Entity must be a vertex");
        case ErrorCode::NotCircularEdge: return textIdTr("Entity must be a circular edge");
        case ErrorCode::NotBRepShape: return textIdTr("Entity must be a shape(BREP)");
        ...
        case ErrorCode::CenterFailure: return textIdTr("Unable to find center of the shape");
        case ErrorCode::NotAllEdges: return textIdTr("All entities must be edges");
        case ErrorCode::NotLinearEdge: return textIdTr("Entity must be a linear edge");
        case ErrorCode::NotAllFaces: return textIdTr("All entities must be faces");
        case ErrorCode::ParallelEdges: return textIdTr("Entities must not be parallel");
        case ErrorCode::BoundingBoxIsVoid: return textIdTr("Bounding box computed is void");
        default: return textIdTr("Unknown error");
        }
    }
};

template <ErrorCode Err>
void throwErrorIf(bool cond)
{
    if (cond)
...
const TopoDS_Shape getShape(const GraphicsOwnerPtr &owner)
{
    ...
    // If scale factor <> 1 then it will cause a crash(exception) in
    // TopoDS_Shape::Move() starting from OpenCascade >= 7.6
...
MeasureDistance MeasureToolBRep::minDistance(const GraphicsOwnerPtr &owner1,
                                             const GraphicsOwnerPtr &owner2) const
{
    return brepMinDistance(getShape(owner1), getShape(owner2));
}

MeasureDistance MeasureToolBRep::centerDistance(const GraphicsOwnerPtr &owner1,
                                                const GraphicsOwnerPtr &owner2) const
{
    return brepCenterDistance(getShape(owner1), getShape(owner2));
}

MeasureAngle MeasureToolBRep::angle(const GraphicsOwnerPtr &owner1,
                                    const GraphicsOwnerPtr &owner2) const
{
...
MeasureCircle MeasureToolBRep::brepCircleFromGeometricEdge(const TopoDS_Edge &edge)
{
    ...
    else
    {
        // Try to create a circle from 3 sample points on the curve
        {
            ...
            const GC_MakeCircle makeCirc(GeomUtils::d0(curve, pnts.Parameter(1)),
...
MeasureCircle MeasureToolBRep::brepCircleFromPolygonEdge(const TopoDS_Edge &edge)
{
    ...
    const GC_MakeCircle makeCirc(polyline->Nodes().First(),
    ...
    const double tol =
        !MathUtils::fuzzyIsNull(polyline->Deflection()) ? 1.5 * polyline->Deflection() : 1e-4;
    ...
    result.isArc =
        !polyline->Nodes().First().IsEqual(polyline->Nodes().Last(), Precision::Confusion());
    result.value = circle;
    return result;
}
...
MeasureDistance MeasureToolBRep::brepMinDistance(const TopoDS_Shape &shape1,
...
MeasureDistance MeasureToolBRep::brepCenterDistance(const TopoDS_Shape &shape1,
                                                    const TopoDS_Shape &shape2)
{
    throwErrorIf<ErrorCode::NotBRepShape>(shape1.IsNull());
    throwErrorIf<ErrorCode::NotBRepShape>(shape2.IsNull());
...
MeasureArea MeasureToolBRep::brepArea(const TopoDS_Shape &shape)
{
    ...
    if (BRepUtils::isGeometric(face))
    {
        ...
        areaResult.middlePnt =
            surface.Value((surface.FirstUParameter() + surface.LastUParameter()) / 2.,
