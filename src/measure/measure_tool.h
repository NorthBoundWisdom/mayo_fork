...
// Input data of a measure service is one or many graphics entities pointed to
// by GraphicsOwner objects
class IMeasureTool
{
public:
    ...
    virtual MeasureDistance minDistance(const GraphicsOwnerPtr &owner1,
                                        const GraphicsOwnerPtr &owner2) const = 0;
    virtual MeasureDistance centerDistance(const GraphicsOwnerPtr &owner1,
                                           const GraphicsOwnerPtr &owner2) const = 0;
    virtual MeasureAngle angle(const GraphicsOwnerPtr &owner1,
                               const GraphicsOwnerPtr &owner2) const = 0;
...
// Union type for all the various measure values returned by IMeasureTool
// services
using MeasureValue = std::variant<MeasureNone, // WARNING: ensure this is the first value type in
                                               // the variant
                                  gp_Pnt, MeasureCircle, MeasureDistance, MeasureAngle,
                                  MeasureLength, MeasureArea, MeasureBoundingBox>;

bool MeasureValue_isValid(const MeasureValue &res);

MeasureValue IMeasureTool_computeValue(const IMeasureTool &tool, MeasureType type,
