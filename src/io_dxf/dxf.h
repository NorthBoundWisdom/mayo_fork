...
enum class StringToErrorMode
{
    Throw = 0x1,
    ReturnErrorValue = 0x2
};

double stringToDouble(const std::string &line,
                      StringToErrorMode errorMode = StringToErrorMode::Throw);

int stringToInt(const std::string &line, StringToErrorMode errorMode = StringToErrorMode::Throw);
...
private:
    ...
    void OnReadArc(double start_angle, double end_angle, double radius, const DxfCoords &c,
                   double z_extrusion_dir, bool hidden);
    void OnReadCircle(const DxfCoords &c, double radius, bool hidden);
    void OnReadEllipse(const DxfCoords &c, const DxfCoords &m, double ratio, double start_angle,
                       double end_angle);
    bool ReadInsert();
    bool ReadDimension();
    bool ReadBlockInfo();
    ...
    void HandleCoordCode(int n, DxfCoords *coords)
    {
        switch (n)
        {
        case XCode: coords->x = mm(DxfPrivate::stringToDouble(m_str)); break;
        case YCode: coords->y = mm(DxfPrivate::stringToDouble(m_str)); break;
        case ZCode: coords->z = mm(DxfPrivate::stringToDouble(m_str)); break;
        }
    }
...
protected:
    ...
    virtual bool setSourceEncoding(const std::string & /*codepage*/)
    {
        return true;
    }
    virtual std::string toUtf8(const std::string &strSource)
    {
        return strSource;
    }

private:
    std::string m_CodePage; // Code Page name from $DWGCODEPAGE or null if
                            // none/not read yet
...
public:
    ...
    virtual void OnReadArc(const DxfCoords &s, const DxfCoords &e, const DxfCoords &c, bool dir,
                           bool hidden) = 0;

    virtual void OnReadCircle(const DxfCoords &s, const DxfCoords &c, bool dir, bool hidden) = 0;

    virtual void OnReadEllipse(const DxfCoords &c, double major_radius, double minor_radius,
                               double rotation, double start_angle, double end_angle, bool dir) = 0;
    ...
    virtual void OnReadDimension(const DxfCoords &s, const DxfCoords &e, const DxfCoords &point,
                                 double rotation) = 0;
