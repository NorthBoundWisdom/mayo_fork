...
public:
    ...
    void setParameters(const DxfReader::Parameters &params)
    {
        m_params = params;
    }
    ...
    void OnReadArc(const DxfCoords &s, const DxfCoords &e, const DxfCoords &c, bool dir,
                   bool hidden) override;
    void OnReadCircle(const DxfCoords &s, const DxfCoords &c, bool dir, bool hidden) override;
    void OnReadEllipse(const DxfCoords &c, double major_radius, double minor_radius,
                       double rotation, double start_angle, double end_angle, bool dir) override;
    void OnReadSpline(const Dxf_SPLINE &spline) override;
    void OnReadInsert(const Dxf_INSERT &ins) override;
    void OnReadDimension(const DxfCoords &s, const DxfCoords &e, const DxfCoords &point,
...
public:
    ...
    {
        this->scaling.setDescription(textIdTr("Scale entities according some factor"));
        this->importAnnotations.setDescription(textIdTr("Import text/dimension objects"));
    ...
    PropertyEnumeration fontNameForTextObjects{this, textId("fontNameForTextObjects"),
                                               &systemFontNames()};
};
...
TDF_LabelSequence DxfReader::transfer(DocumentPtr doc, TaskProgress *progress)
{
    ...
    auto fnAddRootShape =
        [&](const TopoDS_Shape &shape, const std::string &shapeName, TDF_Label layer)
    {
            ...
            if (!comp.IsNull())
            {
                ...
                else
                {
                    for (const Entity &entity : vecEntity)
                    {
                        if (!entity.shape.IsNull())
                        {
                            const TDF_Label entityLabel =
                                shapeTool->AddSubShape(compLabel, entity.shape);
...
void DxfReader::Internal::OnReadPolyline(const Dxf_POLYLINE &polyline)
{
    ...
    else
    {
        ...
        const int nodeCount =
            CppUtils::safeStaticCast<int>(vertices.size() + (isPolylineClosed ? 1 : 0));
...
void DxfReader::Internal::OnReadText(const Dxf_TEXT &text)
{
    ...
    if (!brepFont.Init(fontName.c_str(), Font_FA_Regular,
                       fontHeight /*, Font_StrictLevel_Aliases*/))
    {
    ...
    const DxfCoords &alignPnt =
        applyFirstAlignPnt ? text.firstAlignmentPoint : text.secondAlignmentPoint;
    ...
    // If rotation angle is non-null and x-axis direction defaults to standard Ox
    // then set x-axis so it matches rotation angle
    xAxisDir.Normalize();
...
void DxfReader::Internal::OnReadMText(const Dxf_MTEXT &text)
{
    ...
    /* Font_TextFormatter computes weird ResultWidth() so wrapping is currently
    broken if (text.acadHasColumnInfo && text.acadColumnInfo_Width > 0.) {
...
void DxfReader::Internal::OnReadArc(const DxfCoords &s, const DxfCoords &e, const DxfCoords &c,
...
// Excerpted from FreeCad/src/Mod/Import/App/ImpExpDxf
void DxfReader::Internal::OnReadCircle(const DxfCoords &s, const DxfCoords &c, bool dir,
                                       bool /*hidden*/)
{
    const gp_Pnt p0 = this->toPnt(s);
    const gp_Dir up = dir ? gp::DZ() : -gp::DZ();
...
// Excerpted from FreeCad/src/Mod/Import/App/ImpExpDxf
void DxfReader::Internal::OnReadEllipse(const DxfCoords &c, double major_radius,
                                        double minor_radius, double rotation,
                                        double /*start_angle*/, double /*end_angle*/, bool dir)
{
    const gp_Dir up = dir ? gp::DZ() : -gp::DZ();
    const gp_Pnt pc = this->toPnt(c);
    gp_Elips ellipse(gp_Ax2(pc, up), major_radius * m_params.scaling,
...
void DxfReader::Internal::OnReadInsert(const Dxf_INSERT &ins)
{
    ...
    for (const auto &[k, vecEntity] : m_layers)
    {
        ...
        {
            m_messenger->emitWarning(fmt::format("OnReadInsert('{}') - non-uniform scales aren't "
                                                 "supported({}, {}, {})",
                                                 ins.blockName, ins.scaleFactor.x,
                                                 ins.scaleFactor.y, ins.scaleFactor.z));
        }

        auto fnNonNull = [](double v) { return !MathUtils::fuzzyIsNull(v) ? v : 1.; };
        const double avgScale =
            std::abs(fnNonNull((ins.scaleFactor.x + ins.scaleFactor.y + ins.scaleFactor.z) / 3.));
        if (!MathUtils::fuzzyEqual(avgScale, 1.))
        {
            ...
            {
                m_messenger->emitWarning(
                    fmt::format("OnReadInsert('{}') - scaling failed({}, {}, {})", ins.blockName,
                                ins.scaleFactor.x, ins.scaleFactor.y, ins.scaleFactor.z));
            }
        }
...
void DxfReader::Internal::OnReadDimension(const DxfCoords &s, const DxfCoords &e,
                                          const DxfCoords &point, double rotation)
{
    if (m_params.importAnnotations)
    {
        // TODO
...
// Excerpted from FreeCad/src/Mod/Import/App/ImpExpDxf
OccHandle<Geom_BSplineCurve>
DxfReader::Internal::createSplineFromPolesAndKnots(const Dxf_SPLINE &spline)
{
    ...
    std::cout << "\n    BSplCLib::NbPoles(): "
              << BSplCLib::NbPoles(spline.degree, isPeriodic, occMults);
    std::cout << std::endl;
#endif

    return new Geom_BSplineCurve(occPoles, occWeights, occUniqueKnots, occMults, spline.degree,
                                 isPeriodic);
}

// Excerpted from FreeCad/src/Mod/Import/App/ImpExpDxf
OccHandle<Geom_BSplineCurve>
DxfReader::Internal::createInterpolationSpline(const Dxf_SPLINE &spline)
{
