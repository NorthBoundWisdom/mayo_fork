...
bool compareCircle(const gp_Circ &lhs, const gp_Circ &rhs,
...
void TestMeasure::BRepCircle_Regular_test()
{
    ...
    QVERIFY(circleRes.pntAnchor.IsEqual(GeomUtils::d0(BRepAdaptor_Curve(edge), 0),
                                        Precision::Confusion()));
...
void TestMeasure::BRepCircle_PseudoCircle_test()
{
    ...
    const GC_MakeCircle makeCircle(gp_Ax2(pntCenter, dirNormal), radius);
    GeomConvert_ApproxCurve approxCircle(makeCircle.Value(), Precision::Approximation(), GeomAbs_C1,
                                         2048, 8);
...
void TestMeasure::BRepBoundingBox_Sphere_test()
{
    ...
    QVERIFY(bndBox.cornerMin.IsEqual(gp_Pnt{-sphereRadius, -sphereRadius, -sphereRadius},
                                     Precision::Confusion()));
    QVERIFY(bndBox.cornerMax.IsEqual(gp_Pnt{sphereRadius, sphereRadius, sphereRadius},
                                     Precision::Confusion()));
    ...
    QCOMPARE(double(UnitSystem::cubicMillimeters(bndBox.volume)),
             8 * sphereRadius * sphereRadius * sphereRadius);
}
