...
void TestApp::DocumentFilesWatcher_test()
{
    ...
    docFilesWatcher.signalDocumentFileChanged.connectSlot(
        [&](DocumentPtr changedDoc)
        {
    ...
    auto fnCopyCadFile = [=]
    {
        std_filesystem::copy_file("tests/inputs/cube.ply", cadFilePath,
...
void TestApp::QStringUtils_append_test_data()
{
    ...
    QTest::newRow("locale_c") << QString("1234") << QStringLiteral("12") << QStringLiteral("34")
                              << QLocale::c();
    QTest::newRow("locale_fr") << QString("1234") << QStringLiteral("12") << QStringLiteral("34")
                               << QLocale(QLocale::French);
    QTest::newRow("locale_arabic") << QString("3412") << QStringLiteral("12")
                                   << QStringLiteral("34") << QLocale(QLocale::Arabic);
    QTest::newRow("locale_syriac") << QString("3412") << QStringLiteral("12")
                                   << QStringLiteral("34") << QLocale(QLocale::Syriac);
}
...
void TestApp::QStringUtils_text_test_data()
{
    ...
    const QStringUtils::TextOptions opts_fr_si_2 = {QLocale(QLocale::French, QLocale::France),
                                                    UnitSystem::SI, 2};
    QTest::newRow("c_0.1") << QStringUtils::text(0.1, opts_c_si_2) << QStringLiteral("0.1");
    QTest::newRow("c_0.155") << QStringUtils::text(0.155, opts_c_si_2) << QStringLiteral("0.15");
    QTest::newRow("c_0.159") << QStringUtils::text(0.159, opts_c_si_2) << QStringLiteral("0.16");
    QTest::newRow("fr_1.4995") << QStringUtils::text(1.4995, opts_fr_si_2) << QStringLiteral("1,5");
...
void TestApp::AppUiState_test()
{
    ...
    QCOMPARE(uiState.pageDocuments_isLeftSideBarVisible,
             uiState_read.pageDocuments_isLeftSideBarVisible);
}
